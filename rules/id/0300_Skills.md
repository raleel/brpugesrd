# 3. Keterampilan

Karakter Anda harus membuat banyak keputusan, menghadapi banyak tantangan, dan menyelesaikan banyak tugas selama permainan. Sebagian besar tindakan penting didefinisikan sebagai **keterampilan** (*skills*) yang dapat dicoba oleh karakter Anda. Bagian ini mencantumkan keterampilan yang paling umum, mendefinisikannya, dan menghubungkannya dengan tugas-tugas yang dapat dicoba oleh karakter Anda dalam permainan.

## Mendefinisikan Keterampilan

Sebuah keterampilan mewakili seperangkat pengetahuan dan kemampuan tertentu. Setiap keterampilan dinilai pada skala persentase dari 0 hingga 100 (atau lebih). Ini disebut **peringkat keterampilan** (*skill rating*). Semakin tinggi, semakin baik.

Daftar keterampilan dalam buku ini jauh dari lengkap: permainan yang berbeda mungkin menuntut keterampilan tambahan, dan gamemaster didorong untuk membuat keterampilan baru sesuai keinginan. Selain itu, gamemaster mungkin ingin mengganti nama atau mengklasifikasikan ulang keterampilan, menghapus beberapa, menambahkan spesialisasi baru (dijelaskan di bawah), atau menghapusnya, sebagaimana mestinya.

### Spesialisasi Keterampilan

Banyak keterampilan bersifat luas dan mengharuskan Anda menentukan spesialisasi, aspek yang lebih terfokus dari keterampilan tersebut. Sebagai contoh, Seni adalah kategori luas, sehingga dibagi menjadi spesialisasi seperti Menggambar, Melukis, Memahat, Menulis, dll. Spesialisasi ini dicantumkan setelah nama keterampilan di dalam tanda kurung. Dengan demikian, Senjata Jarak Dekat (Pedang) dan Senjata Jarak Dekat (Tombak) dianggap sebagai dua keterampilan yang berbeda. Atas kebijaksanaan gamemaster, jika karakter Anda tidak memiliki peringkat keterampilan dalam spesialisasi yang sesuai, Anda dapat menggunakan peringkat keterampilan atau spesialisasi terkait yang dibagi dua (dibulatkan ke atas) untuk melakukan lemparan keterampilan.

Sebagai contoh, *karakter Anda dapat menggunakan setengah dari peringkat keterampilan mereka dalam Sains (Astronomi) untuk melakukan lemparan keterampilan yang biasanya memerlukan Sains (Fisika) atau Sains (Matematika), karena ini adalah keterampilan yang terkait. Namun, ketajaman astronomi ini tidak berguna jika gamemaster meminta lemparan keterampilan Sains (Biologi) atau Pengetahuan (Sejarah).*

### Menggunakan Keterampilan

Setiap kali karakter Anda ingin melakukan tindakan atau menyelesaikan tugas yang hasilnya diragukan, Anda harus menyatakan tindakan yang diinginkan. Gamemaster kemudian akan meminta lemparan D100 terhadap peringkat keterampilan yang sesuai. Secara umum, lemparan D100 yang sama dengan atau di bawah peringkat keterampilan yang dimodifikasi memberikan keberhasilan pada tugas tersebut, sementara lemparan yang lebih tinggi dari peringkat tersebut berarti gagal.

Tingkat kesulitan tugas atau pengubah situasional lainnya dapat menyesuaikan peringkat keterampilan ke atas atau ke bawah. Pengubah tingkat kesulitan adalah:

* **Otomatis** tidak memerlukan lemparan
* **Mudah** keterampilan digandakan
* **Rata-rata** tidak ada modifikasi pada keterampilan
* **Sulit** keterampilan dibagi dua
* **Mustahil** tidak ada lemparan, atau peluang 01% atas kebijaksanaan gamemaster

Lihat **Memodifikasi Lemparan Tindakan** untuk informasi lebih lanjut. Jika tidak ada pengubah yang ditunjukkan, anggap lemparan tersebut berada pada tingkat kesulitan Rata-rata.

Dalam kebanyakan kasus, keterampilan yang sesuai untuk tindakan tertentu sudah jelas. Namun, keadaan yang tidak terduga dapat menyebabkan penggunaan keterampilan yang baru. Anda didorong untuk menemukan aplikasi alternatif dari keterampilan karakter Anda, tetapi semua penggunaan keterampilan tersebut harus disetujui oleh gamemaster.

Meskipun banyak keterampilan berorientasi pada tindakan, keterampilan tersebut juga menunjukkan pengetahuan umum tentang jenis tugas yang diatur oleh keterampilan tersebut.

Sebagai contoh, *seorang prajurit abad pertengahan mungkin menggunakan keterampilan Senjata Jarak Dekat (Pedang) alih-alih Menilai untuk menentukan kualitas pedang. Keterampilan Seni Bela Diri mungkin juga digunakan untuk mengetahui tentang berbagai dojo di sebuah kota dan siapa sensei mereka.*

### Lemparan Keterampilan Rahasia atau Tersembunyi

Gamemaster juga dapat melakukan lemparan keterampilan untuk karakter Anda, dalam kasus di mana gamemaster tidak ingin Anda mengetahui hasil lemparan keterampilan atau bahwa lemparan keterampilan sedang dilakukan sama sekali. Gamemaster mungkin ingin menyimpan salinan lembar karakter Anda atau meminta daftar peringkat karakter Anda saat ini dalam keterampilan yang relevan (terutama keterampilan Persepsi) agar lemparan rahasia dapat dilakukan tanpa memberi tahu Anda dan pemain lain bahwa karakter Anda mungkin melewatkan sesuatu ketika lemparan keterampilan gagal. Metode ini memungkinkan gamemaster untuk menggunakan lemparan yang gagal untuk berpotensi menyesatkan atau mengarahkan Anda, yang merupakan taktik berguna untuk lemparan berdasarkan Persepsi atau Mental.

Beberapa lemparan keterampilan yang lebih jelas yang dapat dilakukan secara rahasia, atau tanpa Anda mengetahui hasilnya, adalah sebagai berikut: Menilai, Seni, Menawar, Perintah, Kriya, Penyamaran, Etiket, Bicara Cepat, Permainan, Bersembunyi, Wawasan, Pengetahuan, Bahasa, Mendengar, Literasi, Pengobatan, Navigasi, Pertunjukan, Membujuk, Psikoterapi, Riset, Sains, Perasaan, Kecekatan Tangan, Melihat, Status, Siluman, Strategi, Mengajar, Keterampilan Teknis, dan Melacak. Keterampilan lain juga dapat dilakukan secara rahasia, tergantung pada keadaan.

Lemparan ini tidak harus sepenuhnya rahasia, dan gamemaster dapat memberi tahu Anda bahwa lemparan sedang dilakukan terhadap keterampilan tertentu. Tergantung pada gaya bermain, Anda dan gamemaster dapat memilih untuk melakukan lemparan tersembunyi di mana Anda melempar dadu di area yang tidak dapat Anda lihat jatuhnya, seperti di balik layar gamemaster atau penyembunyian lainnya.

Jika keterampilan berhasil, gamemaster harus memberi tahu Anda bahwa karakter Anda telah berhasil menggunakan keterampilan yang sesuai dan mengizinkan pemeriksaan pengalaman, jika berlaku. Jika lemparan keterampilan gagal, gamemaster harus memberi tahu, memberikan informasi yang salah, atau menahan diri untuk tidak memberi tahu Anda (dan karakter Anda) tentang hasilnya, sebagaimana mestinya.

## Kategori Keterampilan

Keterampilan dibagi menjadi enam kategori keterampilan:

* **Tempur** keterampilan mengukur kemahiran dengan senjata dan manuver tempur.
* **Komunikasi** keterampilan mengatur percakapan, membaca, dan pertukaran antarpribadi.
* **Manipulasi** keterampilan mendefinisikan tugas yang memerlukan koordinasi tangan-mata yang presisi.
* **Mental** keterampilan mengukur pengetahuan spesifik dan penilaian individu.
* **Persepsi** keterampilan mengumpulkan dan menafsirkan informasi dari lingkungan.
* **Fisik** keterampilan mengatur prestasi kekuatan, kelincahan, dan atletik.

Kategori keterampilan berfungsi sebagai panduan umum untuk jenis tindakan yang dapat diselesaikan oleh suatu keterampilan, serta menawarkan kerangka kerja bagi Anda dan gamemaster untuk membuat keterampilan baru. Kategori keterampilan menghubungkan peringkat keterampilan karakter Anda dengan skor karakteristik mereka yang paling berpengaruh melalui bonus kategori, sebuah sistem opsional.

### Keterampilan Baru

Ada baris kosong di bagian keterampilan pada lembar karakter. Ini untuk keterampilan baru yang mungkin Anda atau gamemaster buat. Sebagai contoh, *Bab Empat: Kekuatan* memperkenalkan keterampilan baru opsional yang disebut Proyeksi, yang digunakan bersama dengan banyak kekuatan. Anda dapat menyarankan dan mendefinisikan keterampilan baru, tetapi hanya gamemaster yang dapat mengizinkan penggunaannya dalam permainan Anda.

### Mengubah Daftar Keterampilan

Sama mudahnya seperti Anda dan gamemaster dapat membuat keterampilan baru, Anda dapat mengganti nama keterampilan agar sesuai dengan latar tertentu. Sebagai contoh, Manipulasi Halus dapat diganti namanya menjadi Merancang atau Membuka Kunci untuk lingkungan abad pertengahan atau fantasi. Gamemaster Anda mungkin juga memilih untuk menghilangkan keterampilan tertentu sama sekali jika tidak sesuai dengan gaya permainan mereka. Sangat disarankan agar perubahan semacam itu ditentukan sebelum pembuatan karakter dan dibuat jelas, sehingga Anda tidak memilih keterampilan yang mungkin dikesampingkan atau diubah oleh gamemaster.

> ### Bonus Kategori Keterampilan
>
> Seperti yang dijelaskan dalam **Bonus Kategori Keterampilan**, karakter Anda menerima bonus atau penalti pada kategori keterampilan berdasarkan skor karakteristik mereka. Terapkan bonus kategori ke setiap keterampilan dalam kategori tersebut, sebagai tambahan dari peluang dasar yang tercantum, jika ada, sebagaimana tercantum dalam deskripsi keterampilan. Lihat **Tabel Bonus Kategori Keterampilan** untuk rumus yang digunakan untuk menghitung bonus kategori keterampilan.

## Peluang Dasar

Kebanyakan orang yang cukup sehat dan bugar secara fisik dapat mengayunkan tongkat, memanjat pohon, atau berbicara dalam bahasa ibu mereka. Oleh karena itu, setiap keterampilan memiliki peluang dasar yang terkait dengannya: karakter Anda dianggap memiliki peringkat dasar ini dalam keterampilan tersebut sebelum poin keterampilan tambahan ditambahkan. Jika bonus kategori keterampilan (lihat **Bonus Kategori Keterampilan**) digunakan, tambahkan (atau kurangi) dari peluang dasar setiap keterampilan.

Peluang dasar suatu keterampilan sangat bergantung pada era kampanye: karakter dalam kampanye yang berlatar di Eropa abad pertengahan memiliki peringkat dasar yang lebih tinggi dalam Pengetahuan (Agama) daripada warga negara AS modern, sementara warga negara AS mungkin memiliki keunggulan dalam keterampilan Medis karena pelatihan pertolongan pertama dasar di sekolah umum dan tempat kerja. Setiap deskripsi keterampilan mencantumkan beberapa peluang dasar untuk era yang berbeda, sebagaimana mestinya. Gamemaster harus merasa bebas untuk mengubah peluang dasar keterampilan agar sesuai dengan latar yang berbeda.

> #### Bertindak tanpa Keterampilan
>
> Terkadang karakter Anda harus mencoba tindakan menggunakan keterampilan tanpa peringkat keterampilan (0%). Jika gamemaster setuju bahwa keberhasilan dalam tugas tersebut dimungkinkan, lakukan lemparan menggunakan salah satu opsi ini:
>
> * **Bonus Kategori Keterampilan:** Jika menggunakan sistem kategori keterampilan opsional, gunakan pengubah kategori keterampilan sebagai peluang dasar. Bakat alami terkadang dapat mengatasi kurangnya pelatihan. Jika berhasil dan keterampilan dapat ditingkatkan melalui pengalaman, pemeriksaan pengalaman harus dilakukan.
>
> * **Peluang Liar:** Coba, tetapi hanya dengan 1% peluang sukses atau upaya terakhir 'Hail Mary' sebesar POW×1 jika keberuntungan buta bisa menjadi faktor, dengan persetujuan gamemaster. Tidak ada pemeriksaan pengalaman yang diizinkan untuk ini, karena ini adalah keberuntungan buta dibandingkan dengan segala jenis keterampilan atau bakat alami.

### Mengukur Peringkat Keterampilan

Peringkat keterampilan karakter Anda menentukan kemampuan mereka selama permainan. Namun, apa arti peringkat keterampilan tertentu dalam istilah yang lebih umum? Gunakan pedoman berikut sebagai panduan untuk kemahiran. Pada peringkat keterampilan tertentu dan dalam situasi yang tidak kritis atau tidak dramatis, lemparan mungkin bahkan tidak diperlukan—dalam kondisi rutin, karakter pada tingkat kompetensi tertentu dapat secara teratur melakukan keterampilan ini tanpa perlu melempar untuk sukses, meskipun keberhasilan *Otomatis* tidak mendapatkan pemeriksaan pengalaman.

##### Tabel Peringkat Keterampilan

| Peringkat Keterampilan | Kompetensi | Deskripsi |
| :---- | :---- | :---- |
| 00–05% | Pemula | Seorang amatir lengkap dengan sedikit harapan untuk sukses bahkan pada tugas yang Mudah. Anda harus melempar setiap kali karakter Anda mencoba keterampilan ini, baik dalam situasi yang penuh tekanan atau dalam kondisi ideal. |
| 06–25% | Neofit | Baik pemula atau dengan sedikit pengetahuan tentang subjek yang dihadapi. Seperti yang mereka katakan, 'Tahu cukup untuk menjadi berbahaya.' Berhasil sesekali dan mungkin menjadi terlalu percaya diri sebagai hasilnya. Lempar setiap kali karakter Anda mencoba keterampilan ini, bahkan jika tugasnya Mudah. |
| 26–50% | Amatir | Sedikit bakat, beberapa pelatihan, atau hobi. Karakter Anda hampir tidak memenuhi syarat tetapi dapat dipercaya dengan tugas Otomatis atau Mudah. Dalam situasi yang tidak menekan, karakter Anda dapat melakukan aktivitas rutin menggunakan keterampilan ini tanpa perlu melempar. |
| 51–75% | Profesional | Pada 51%, karakter Anda dapat dengan andal mencari nafkah menggunakan keterampilan tersebut. Mereka biasanya tidak perlu khawatir gagal dalam tugas Mudah dan cukup berpengalaman atau mahir untuk menjadi pemimpin atau manajer orang lain. 51% kira-kira setara dengan gelar sarjana dalam disiplin ilmu tertentu. Sebagian besar keterampilan tidak dapat ditingkatkan di atas 75% melalui pendidikan atau pelatihan saja: karakter Anda harus maju lebih jauh melalui penggunaan praktis. Karakter Anda biasanya dapat melakukan aktivitas rutin atau bahkan kompleks yang berkaitan dengan keterampilan tersebut dan Anda hanya diharuskan untuk melempar ketika ada elemen risiko, atau ketika kemungkinan kegagalan bersifat dramatis. |
| 76–90% | Ahli | Keahlian tingkat lanjut di bidang tertentu: karakter Anda memiliki peluang yang wajar untuk sukses bahkan pada tugas yang Sulit. Hanya profesional berdedikasi yang mencapai penguasaan keterampilan tertentu, dan mereka yang mencapainya dihormati dan diandalkan karena keahlian mereka. Keterampilan dalam rentang ini sesuai dengan gelar lanjutan (Master atau Ph.D.), atau pengalaman bertahun-tahun. Kebanyakan karakter rata-rata tidak dapat memulai permainan dengan keterampilan lebih tinggi dari 75%. Karakter Anda seharusnya dapat melakukan tindakan kompleks dan menantang yang berkaitan dengan keterampilan ini dalam kondisi rutin tanpa melempar, dengan asumsi mereka meluangkan waktu dan memiliki fasilitas dan/atau peralatan yang tepat. |
| 91% atau lebih tinggi | Master | Sedikit yang mencapai tingkat penguasaan ini. Hanya tugas yang paling sulit yang berada di luar kemampuan mereka, dan keahlian mereka kemungkinan telah membuat para master ini terkenal di bidangnya dan terkadang di dunia pada umumnya. Dalam situasi rutin, karakter Anda dapat melakukan keajaiban, dan Anda akan jarang perlu melempar dadu kecuali pada tugas yang paling sulit atau dalam situasi yang mengerikan. Jika aturan Kewarasan opsional digunakan dalam kampanye, mencapai peringkat 91% dalam keterampilan secara instan memulihkan 2D6 poin SAN, mewakili kepercayaan diri dan disiplin yang terkait dengan penguasaan keterampilan (ini adalah bonus satu kali). Jika karakter Anda memulai dengan peringkat keterampilan ini atau lebih tinggi, tidak ada perolehan SAN. |

### Peringkat Keterampilan di Atas 100%

Bonus pengalaman karakter Anda memungkinkan untuk meningkatkan peringkat keterampilan di atas 100%. Keterampilan yang lebih tinggi dari 100% menggunakan sistem standar untuk menghitung peluang keberhasilan khusus atau kritis, serta menawarkan peluang sukses yang lebih baik pada tugas yang Sulit atau dalam menghadapi pengubah situasional negatif. Selain itu, jika karakter Anda memiliki keterampilan tempur lebih tinggi dari 100%, mereka dapat melakukan beberapa serangan dalam satu putaran tempur, seperti yang dijelaskan dalam **Serangan dan Tangkisan di Atas 100%**.

### Daftar Keterampilan berdasarkan Kategori

Daftar lengkap keterampilan yang dibagi berdasarkan kategori berikut.

| Tempur | Komunikasi | Manipulasi | Mental |
| :---- | :---- | :---- | :---- |
| Artileri (berbagai) | Menawar | Kriya (berbagai) | Menilai |
| Berkelahi | Perintah | Kerajinan (berbagai) | Pertolongan Pertama |
| Senjata Energi (berbagai) | Penyamaran | Penghancuran | Permainan |
| Senjata Api (berbagai) | Etiket (berbagai) | Manipulasi Halus | Pengetahuan (berbagai) |
| Bergulat | Bicara Cepat | Mesin Berat (berbagai) | Literasi (Opsional) |
| Senjata Berat (berbagai) | Pertunjukan | Perbaikan (berbagai) | Pengobatan |
| Seni Bela Diri | Membujuk | Kecekatan Tangan | Psikoterapi |
| Senjata Jarak Dekat (berbagai) | Status | | Sains (berbagai) |
| Senjata Proyektil (berbagai) | Mengajar | | Strategi |
| Tangkisan (berbagai) | | | Keterampilan Teknis (berbagai) |
| Perisai | | | |

| Persepsi | Fisik | Fisik |
| :---- | :---- | :---- |
| Wawasan | Memanjat | Proyeksi |
| Mendengar | Menghindar | Menunggang (berbagai) |
| Navigasi | Mengemudi (berbagai) | Siluman |
| Riset | Terbang | Berenang |
| Perasaan | Bersembunyi | Melempar |
| Melihat | Melompat | |
| Melacak | Pilot (berbagai) | |

### Daftar Keterampilan Alfabetis

Daftar alfabetis dari semua keterampilan di bagian ini berikut. Setiap entri mencantumkan kategori keterampilan, dan peluang dasarnya.

| Keterampilan | Kategori | Peluang Dasar % |
| :---- | :---- | :---- |
| Menilai | Mental | 15% |
| Seni (berbagai) | Manipulasi | 05% |
| Artileri (berbagai) | Tempur | per spesialisasi senjata |
| Menawar | Komunikasi | 05% |
| Berkelahi | Tempur | 25% |
| Memanjat | Fisik | 40% |
| Perintah | Komunikasi | 05% |
| Kerajinan (berbagai) | Manipulasi | 05% |
| Penghancuran | Manipulasi | 01% |
| Penyamaran | Komunikasi | 01% |
| Menghindar | Fisik | DEX×2 |
| Mengemudi (berbagai) | Fisik | 20% atau 01% |
| Senjata Energi (berbagai) | Tempur | per spesialisasi senjata |
| Etiket (berbagai) | Komunikasi | 05% |
| Bicara Cepat | Komunikasi | 05% |
| Manipulasi Halus | Manipulasi | 05% |
| Senjata Api (berbagai) | Tempur | per spesialisasi senjata |
| Pertolongan Pertama | Mental | 30% |
| Terbang | Fisik | DEXx4 atau 1/2 DEX |
| Permainan | Mental | INT+POW |
| Bergulat | Tempur | 25% |
| Mesin Berat (berbagai) | Manipulasi | 01% |
| Senjata Berat (berbagai) | Tempur | per spesialisasi senjata |
| Bersembunyi | Fisik | 10% |
| Wawasan | Persepsi | 05% |
| Melompat | Fisik | 25% |
| Pengetahuan (berbagai) | Mental | 05% atau 00% |
| Bahasa (berbagai) | Komunikasi | INT (atau EDU)×5 atau 00% |
| Mendengar | Persepsi | 25% |
| Literasi (opsional) | Mental | 00% atau setara dengan Bahasa |
| Seni Bela Diri | Tempur | 01% |
| Pengobatan | Mental | 05% |
| Senjata Jarak Dekat (berbagai) | Tempur | per spesialisasi senjata |
| Senjata Proyektil (berbagai) | Tempur | per spesialisasi senjata |
| Navigasi | Persepsi | 10% |
| Tangkisan (berbagai) | Tempur | per spesialisasi senjata |
| Pertunjukan | Komunikasi | 05% |
| Membujuk | Komunikasi | 15% |
| Pilot (berbagai) | Fisik | 01% |
| Proyeksi | Fisik | DEX×2 |
| Psikoterapi | Mental | 01% atau 00% |
| Perbaikan (berbagai) | Manipulasi | 15% |
| Riset | Persepsi | 25% |
| Menunggang (berbagai) | Fisik | 05% |
| Sains (berbagai) | Mental | 01% |
| Perasaan | Persepsi | 10% |
| Perisai | Tempur | per perisai |
| Kecekatan Tangan | Manipulasi | 05% |
| Melihat | Persepsi | 25% |
| Status | Komunikasi | 15% atau berbagai |
| Siluman | Fisik | 10% |
| Strategi | Mental | 01% |
| Berenang | Fisik | 25% |
| Mengajar | Komunikasi | 10% |
| Keterampilan Teknis (berbagai) | Mental | 05% |
| Melempar | Fisik | 25% |
| Melacak | Persepsi | 10% |

> ### Augmentasi dan Keterampilan Komplementer
>
> Beberapa keterampilan terkait erat dengan yang lain, dan keahlian di satu bidang dapat menguntungkan bidang lainnya. Dengan izin gamemaster Anda, karakter Anda dapat untuk sementara menambahkan 1/5 dari peringkat Anda dalam **keterampilan komplementer** ke peringkat Anda dalam keterampilan lain untuk lemparan keterampilan. Banyak penggunaan komplementer dicatat dalam deskripsi keterampilan berikut. Anda dan pemain lain pasti akan menemukan lebih banyak lagi.
>
> Hanya satu keterampilan yang boleh menjadi komplementer bagi keterampilan lain saat digunakan untuk membantu lemparan tertentu. Jika beberapa keterampilan mungkin berlaku atau digunakan untuk mencoba mendapatkan bonus ini, manfaatnya tidak menumpuk—pemain dan gamemaster harus memilih bonus tertinggi untuk digunakan, atau mana yang paling sesuai dengan situasi yang dihadapi.
>
> Jika lemparan keterampilan utama berhasil, karakter Anda hanya menerima pemeriksaan pengalaman pada keterampilan utama, bukan pada keterampilan komplementer yang digunakan.
>
> Sebagai contoh, *karakter Anda memiliki keterampilan Pengobatan 65% dan Sains (Farmasi) 40%. Mereka mencoba merawat pasien yang sakit. Selama karakter Anda memiliki persediaan medis dan obat-obatan yang cukup, mereka dapat menambahkan 8% (1/5 dari peringkat Sains(Farmasi) mereka) ke peringkat keterampilan Pengobatan, untuk peringkat yang dimodifikasi sebesar 73%. Karakter Anda tidak dapat menambahkan 1/5 dari keterampilan Pertolongan Pertama atau Sains (Biologi) mereka di atas bonus Farmasi. Dalam kasus ini, gamemaster memutuskan bahwa keterampilan Sains (Farmasi) adalah yang paling tepat untuk bonus tersebut.*
>
> **Augmentasi** terhadap keterampilan serupa tetapi bekerja dengan cara yang sedikit berbeda. Jika gamemaster Anda mengizinkannya, Anda dapat mencoba lemparan satu keterampilan komplementer untuk mendukung, atau menambah, lemparan keterampilan utama lainnya.
>
> * Jika lemparan keterampilan augmentasi berhasil, Anda dapat menyesuaikan tingkat kesulitan keterampilan utama sebanyak satu tingkat, seperti mengubah lemparan *Sulit* menjadi *Rata-rata*, atau tugas *Rata-rata* menjadi *Mudah*.
>
> * Ini tidak dapat dilakukan secara bersamaan dengan bonus keterampilan komplementer, bagaimanapun, dan hanya satu tingkat penyesuaian yang mungkin dilakukan.
>
> * Jika augmentasi gagal, keterampilan utama disesuaikan satu tingkat ke arah yang berlawanan, seperti keterampilan *Normal* menjadi *Sulit* karena kebingungan atau informasi yang bertentangan.
>
> * Jika berhasil dengan lemparan keterampilan augmentasi, Anda dapat memeriksanya untuk pengalaman seperti biasa, serta dengan keterampilan utama. Jika lemparan utama gagal, keterampilan augmentasi tidak menerima pemeriksaan pengalaman.
>
> Anda tidak dapat menambah keterampilan dan menggunakan bonus keterampilan komplementer secara bersamaan untuk lemparan keterampilan yang sama.

> ### Mendeskripsikan Hasil Keberhasilan
>
> Umumnya, hasil lemparan keterampilan sudah jelas, tetapi jika ragu, gunakan pedoman berikut saat menentukan hasil:
>
> * **Kegagalan Mutlak** biasanya berarti kebalikan dari hasil yang diinginkan telah dicapai, memberikan informasi yang menyesatkan, menciptakan semacam kemunduran, atau bahkan menempatkan karakter pada posisi yang tidak menguntungkan.
>
> * **Kegagalan** berarti hasil yang diinginkan tidak tercapai.
>
> * **Keberhasilan** normal berarti tujuan yang wajar dan rata-rata telah tercapai. Gamemaster adalah penentu akhir dari hasil, tetapi efeknya harus jelas.
>
> * **Keberhasilan khusus** memiliki dua kali lipat efektivitas keberhasilan reguler dan/atau memberikan informasi tambahan tentang subjek.
>
> * **Keberhasilan kritis** memiliki tiga kali lipat efektivitas keberhasilan reguler, dan/atau memberikan informasi yang cukup tentang subjek.
>
> Seperti dicatat di atas, keterampilan di mana hasil sangat spesifik dan di mana detail diperlukan, telah mempertahankan hasil yang ditentukan.

## Deskripsi Keterampilan

Daftar keterampilan yang komprehensif dan alfabetis berikut. Setiap entri berisi informasi berikut:

* **Nama Keterampilan:** Nama keterampilan.
* **Peluang Dasar:** Peringkat keterampilan yang dimiliki semua karakter dalam keterampilan tersebut, dispesialisasikan berdasarkan era jika berlaku.
* **Kategori:** Kategori keterampilan di mana keterampilan tersebut berada, untuk tujuan klasifikasi umum dan bonus kategori opsional.
* **Deskripsi:** Deskripsi singkat tentang jenis tugas yang dapat diselesaikan menggunakan keterampilan tersebut.
* **Spesialisasi:** Daftar spesialisasi keterampilan yang berbeda, jika ada. Tidak semua keterampilan dengan spesialisasi memiliki spesialisasi yang diuraikan dalam daftar poin—hanya yang bersifat lebih mekanis, seperti keterampilan Tempur, yang dirinci dengan cara ini.
* **Efek:** Hasil lemparan keterampilan, untuk setiap tingkat keberhasilan, jika diperlukan. Singkatnya, hasil keterampilan adalah Kegagalan Mutlak (hasil terburuk yang mungkin, biasanya lemparan 99 atau 00), kegagalan (lemparan di atas peringkat keterampilan Anda), keberhasilan (di bawah peringkat keterampilan Anda), keberhasilan khusus (1/5 atau kurang dari peringkat keterampilan Anda), atau keberhasilan kritis (1/20 dari peringkat keterampilan Anda). *Bab Lima: Sistem* menjelaskan ini secara rinci. Lihat juga bilah sisi Mendeskripsikan Hasil Keberhasilan.
* **Catatan Latar:** Informasi tambahan tentang keterampilan dalam latar yang berbeda. Jika entri ini tidak ada, keterampilan berfungsi tanpa perubahan di hampir semua latar.
* **Catatan Sistem:** Aturan atau sistem yang relevan, jika ada.

### Menilai

**Peluang Dasar:** 15%

**Kategori:** Mental

Gunakan keterampilan ini untuk memperkirakan nilai suatu objek, baik itu lukisan lama, batu permata, senjata, atau kuda tunggangan. Kegagalan berarti mereka tidak tahu, sementara Kegagalan Mutlak memberikan informasi palsu. Keberhasilan khusus atau kritis memberikan informasi tambahan.

### Seni (berbagai)

**Peluang Dasar:** 05%

**Kategori:** Manipulasi

Gunakan keterampilan ini untuk menciptakan karya seni dalam cat atau patung, atau untuk menyusun cerita, puisi, atau karya musik. Keberhasilan kritis atau khusus memberikan karya klasik abadi dengan nilai dan prestise yang jauh lebih tinggi, kegagalan adalah karya amatir biasa, dan Kegagalan Mutlak dapat merusak reputasi penciptanya. Anda akan menggunakan Pertunjukan untuk akting, menari, bernyanyi, atau memainkan alat musik.

**Spesialisasi:** Kaligrafi, Menyusun, Seni Konseptual, Seni Digital, Menggambar, Melukis, Fotografi, Puisi, Patung, Membuat Sketsa, Menulis Lagu, Menulis, dll.

**Catatan Latar:** Seni bersifat universal, meskipun mungkin memiliki spesialisasi khusus era atau latar.

### Artileri (berbagai)

**Peluang Dasar:** Sesuai spesialisasi senjata

**Kategori:** Tempur

Gunakan keterampilan ini untuk menembakkan senjata medan besar dan senjata kendaraan berat, pada dasarnya senjata apa pun yang terlalu besar untuk digerakkan atau dimanipulasi oleh satu orang, seperti meriam, trebuchet, atau ballista.

**Spesialisasi:** Spesialisasi setara dengan jenis. Lihat **Kelas Senjata** di *Bab Delapan: Peralatan* untuk jenis artileri.

**Efek:** Lihat **Matriks Serangan dan Pertahanan** dan **Tabel Kegagalan Mutlak Senjata Proyektil** untuk efek spesifik.

**Catatan Latar:** Cocok untuk banyak latar, mulai dari mesin pengepungan hingga artileri modern atau futuristik dan senjata kendaraan. Di sebagian besar latar, latar belakang militer diperlukan untuk mempelajari keterampilan ini.

**Catatan Sistem:** Banyak senjata besar memerlukan kru untuk dioperasikan; mencoba menggunakan satu sendirian adalah tindakan Sulit atau mungkin Mustahil. Mungkin diperlukan beberapa putaran atau bahkan giliran untuk memuat dan menyiapkan sistem senjata di antara tembakan. Banyak senjata besar menggunakan tembakan otomatis atau menembakkan persenjataan peledak. Lihat **Tembakan Otomatis** dan **Ledakan**. Semua tembakan pergi ke suatu tempat; gamemaster harus memutuskan di mana tembakan peledak atau energi yang meleset berdampak.

### Menawar

**Peluang Dasar:** 05%

**Kategori:** Komunikasi

Gunakan keterampilan ini untuk menawar harga, dan setiap kali pendapat yang berlawanan bertemu dalam negosiasi dan kompromi. Gunakan lemparan keterampilan yang berlawanan untuk memutuskan kesepakatan penting, dan lemparan keterampilan sederhana untuk upaya sepele (menghemat uang untuk peralatan atau penginapan, misalnya). Menawar adalah tentang kompromi: gunakan Bicara Cepat atau Membujuk untuk menyelesaikan argumen yang benar-benar dimenangkan atau kalah. Kegagalan Mutlak mungkin mengubah pedagang menjadi musuh, keberhasilan berarti Anda mendapatkan kesepakatan, dan hasil yang lebih baik menghasilkan diskon yang lebih tinggi atau kondisi yang lebih menguntungkan bagi pihak Anda.

### Berkelahi

**Peluang Dasar:** 25%

**Kategori:** Tempur

Gunakan keterampilan ini untuk pertarungan tanpa senjata: tendangan, pukulan, tandukan kepala, dll.

**Efek:** Lihat **Matriks Serangan dan Pertahanan** dan **Tabel Kegagalan Mutlak Serangan dan Tangkisan Senjata Alami**.

**Catatan Sistem:** Berkelahi dapat digunakan untuk menangkis (lihat Tangkisan) tetapi tangkisan yang berhasil menimbulkan kerusakan senjata minimum pada lengan atau kaki yang menangkis, sebagaimana mestinya. Menggunakan Berkelahi terhadap musuh yang telah bergulat dengan karakter Anda (lihat Bergulat) adalah Mudah. Digunakan bersama dengan Seni Bela Diri dan dengan kekuatan Senjata Alami dan Pertarungan Tanpa Senjata dari *Bab Empat: Kekuatan*.

### Memanjat

**Peluang Dasar:** 40%

**Kategori:** Fisik

Gunakan keterampilan ini untuk memanjat dinding, pohon, tebing, atau permukaan lainnya, naik atau turun. Pegangan tangan dan kaki yang cukup diasumsikan ada: ubah tingkat kesulitan berdasarkan kondisi atau permukaan yang dipanjat. Kegagalan Mutlak berarti Anda jatuh dengan keras (lihat **Jatuh**), gagal berarti Anda terjebak atau Anda jatuh (pilihan gamemaster, berdasarkan keadaan), dan keberhasilan yang unggul memungkinkan Anda memanjat lebih cepat dan lebih jauh.

**Catatan Sistem:** Umumnya, memanjat adalah 1/5 dari laju MOV, dan hewan dengan kemampuan memanjat alami dapat bergerak dengan laju MOV penuh mereka. Memanjat tangga adalah Otomatis. Tangga atau pohon yang sangat tinggi adalah Mudah. Memanjat tanpa menggunakan kaki karakter Anda adalah Sulit. Untuk pendakian yang diperpanjang, gamemaster harus meminta lemparan keterampilan sekali setiap menit, atau 10 meter pendakian. Karakter Anda tidak dapat melakukan tindakan lain saat memanjat, dan semua tindakan yang dilakukan saat memanjat adalah Sulit, dengan Kegagalan Mutlak apa pun yang menyebabkan jatuh.

### Perintah

**Peluang Dasar:** 05%

**Kategori:** Komunikasi

Gunakan keterampilan ini untuk menginspirasi, mengarahkan, dan mengoordinasikan bawahan, membimbing mereka dalam tugas-tugas kompleks dan meningkatkan efektivitas kelompok. Perwira militer, bangsawan, politisi, dan eksekutif bisnis dibuat atau dihancurkan berdasarkan kemampuan mereka untuk mengelola dan memimpin. Kepribadian, suara yang kuat, dan kemauan keras adalah alat komando yang tak ternilai. Kegagalan Mutlak memicu pemberontakan, gagal berarti tidak menginspirasi dan membingungkan, dan keberhasilan memungkinkan Anda menambahkan 1/5 keterampilan Perintah Anda sebagai bonus keterampilan komplementer kepada semua bawahan Anda untuk setiap putaran Anda memerintah mereka, selama mereka mengikuti perintah Anda. Lempar setiap putaran dalam pertempuran, atau setiap giliran dalam pertempuran massal.

**Catatan Sistem:** Digunakan terutama dalam pertempuran, meskipun dapat diperluas ke negosiasi korporat, dll. Artileri, Senjata Berat, Senjata Jarak Dekat, Senjata Proyektil, dan Strategi adalah keterampilan yang paling tepat untuk menerima manfaat ini.

### Kerajinan (berbagai)

**Peluang Dasar:** 05%

**Kategori:** Manipulasi

Gunakan keterampilan ini untuk membangun atau membuat barang, mulai dari kursi hingga makanan hingga rumah. Alat khusus sering diperlukan untuk keterampilan ini, dan dibutuhkan waktu serta bahan mentah. Keterampilan ini juga dapat digunakan untuk mengidentifikasi bagaimana suatu objek dibuat, dan mungkin mengidentifikasi pembuat yang membuatnya. Kegagalan Mutlak menghancurkan bahan dan tidak menghasilkan apa pun yang berguna, keberhasilan berarti berhasil, dan kualitas keberhasilan yang lebih baik menghasilkan versi barang yang dibuat yang lebih halus, kokoh, atau berharga, mungkin dengan kualitas tambahan seperti +10% atau +20% untuk kemanjurannya saat digunakan, poin hit yang lebih tinggi, dll. Ini tentang fungsi dan kepraktisan: membuat objek semata-mata untuk tujuan estetika termasuk dalam Seni.

**Spesialisasi:** Pandai Besi, Pertukangan, Keramik, Memasak, Pengolahan Kulit, Mengunci, Metalurgi, Pemasangan Batu, dan lainnya sebagaimana mestinya.

**Catatan Sistem:** Menggunakan Kerajinan tanpa alat yang tepat adalah Sulit. Lihat *Bab Delapan: Peralatan* untuk informasi lebih lanjut tentang membuat, memodifikasi, atau memperbaiki peralatan. Memiliki pakaian atau perlengkapan yang bagus bahkan dapat meningkatkan CHA saat barang tersebut dikenakan atau dipajang.

### Penghancuran

**Peluang Dasar:** 01%

**Kategori:** Manipulasi

Gunakan keterampilan ini untuk menyimpan, mengangkut, memasang, dan meledakkan bahan peledak, mulai dari tong mesiu hingga dinamit, koktail Molotov, bahan peledak plastik canggih, dan bom anti-materi. Siapa pun bisa meledakkan bom: ini untuk mencapai efek spesifik dengan ledakan; merobohkan bangunan dengan aman, menghancurkan jembatan, atau meledakkan pintu brankas tanpa merusak isinya. Penghancuran juga mencakup pengetahuan tentang aksesori yang sesuai untuk bahan peledak, termasuk kabel primer, sekering, dan detonator. Gunakan Sains (Kimia) untuk memproduksi senyawa peledak atau Keterampilan Teknis untuk membuat bahan peledak lainnya. Kegagalan Mutlak membuat segalanya meledak di wajah Anda, gagal berarti efek yang diinginkan tidak tercapai (misfire atau dud), dan keberhasilan yang unggul menghasilkan hasil yang lebih mengesankan, baik kerusakan lebih banyak, kerusakan tambahan lebih sedikit, efek khusus seperti Knockback, area efek tambahan, dll.

**Catatan Sistem:** Dengan peringkat keterampilan 60%+, penggunaan normal keterampilan ini secara otomatis berhasil. Bahan peledak yang tidak stabil (dinamit berkeringat) mungkin memerlukan lemparan Penghancuran setiap putaran saat sedang ditangani atau digunakan. Menghancurkan struktur besar (jembatan, bendungan, bunker, atau bangunan) adalah Sulit kecuali karakter Anda memiliki akses ke cetak biru atau rencana target. Mencoba menggunakan keterampilan ini tanpa alat yang sesuai (sekering, dll.), atau menggunakan bahan peledak di luar bidang keahlian normal karakter Anda mungkin Sulit atau bahkan Mustahil. Beberapa jenis bahan peledak seperti granat mungkin digunakan tanpa keterampilan ini. Lihat *Bab Delapan: Peralatan* untuk bahan peledak dan efeknya.

### Penyamaran

**Peluang Dasar:** 01%

**Kategori:** Komunikasi

Gunakan keterampilan ini untuk menggunakan postur, kostum, suara, dan trik lain untuk muncul sebagai orang yang berbeda, atau jenis orang yang berbeda. Cahaya redup, riasan teater, dan perubahan wajah yang signifikan (potongan rambut atau wig, janggut palsu, atau cukuran cepat) dapat menambahkan bonus +20%, atas kebijaksanaan gamemaster. Penonton yang memeriksa karakter Anda yang menyamar atau terlibat dalam percakapan dengan mereka dapat melakukan lemparan keterampilan Melihat atau Wawasan yang berlawanan terhadap lemparan Penyamaran karakter Anda untuk melihat tipu muslihat tersebut. Karakter Anda yang menyamar dapat menggunakan keterampilan Bicara Cepat mereka untuk lemparan yang berlawanan jika mereka melibatkan penonton dalam percakapan, mengalihkan perhatian mereka dari pemeriksaan ketat. Kegagalan Mutlak sangat jelas, gagal tidak menipu siapa pun yang melihat dua kali, dan keberhasilan yang unggul bahkan dapat menambahkan bonus komplementer pada lemparan Komunikasi terhadap mereka yang tertipu, atau membuat deteksi menjadi lebih sulit.

**Catatan Sistem:** Menggunakan keterampilan ini untuk meniru jenis orang tertentu (prajurit, polisi, dll.) adalah tindakan normal. Meniru seseorang yang sangat berbeda (spesies berbeda, seseorang yang jauh lebih tinggi atau lebih pendek, anggota gender lawan) atau orang terkenal tertentu (bangsawan, selebriti, dll.), atau mencoba penyamaran tanpa pakaian atau properti yang sesuai adalah Sulit. Gamemaster dapat memberikan penalti tambahan untuk hambatan budaya atau bahasa, atau jika beberapa kondisi Sulit berlaku. Terlalu berlebihan mungkin hanya Mustahil.

### Menghindar

**Peluang Dasar:** DEX×2

**Kategori:** Fisik

Pertahanan terbaik dalam pertempuran, seperti yang diketahui oleh setiap petarung, adalah tidak berada di sana. Menghindar memungkinkan karakter Anda untuk menghindari serangan yang masuk, menggunakan kelincahan untuk menghindari atau meminimalkan cedera.

**Efek:** Lemparan Menghindar yang berhasil memodifikasi hasil serangan yang berhasil, mengubahnya menjadi kegagalan. Hasil khusus untuk lemparan Menghindar memodifikasi serangan dua langkah (keberhasilan khusus menjadi kegagalan), dan keberhasilan kritis pada lemparan Menghindar memodifikasi serangan yang berhasil tiga langkah (serangan kritis menjadi kegagalan). Lihat **Matriks Serangan dan Pertahanan** dan **Tabel Kegagalan Mutlak Serangan dan Tangkisan Senjata Alami** untuk efek spesifik. Lemparan Menghindar yang berhasil tidak dapat mengurangi keberhasilan serangan yang masuk lebih rendah dari kegagalan.

**Catatan Sistem:** Seperti yang dijelaskan dalam **Menghindar** di *Bab Enam: Tempur*, karakter Anda dapat Menghindar atau Menangkis dalam satu putaran tempur tetapi tidak dapat melakukan keduanya. Jika karakter Anda diserang oleh banyak musuh, mereka dapat melakukan beberapa upaya Menghindar dalam satu putaran, tetapi setiap lemparan setelah yang pertama dibuat dengan penalti kumulatif –30% pada peringkat keterampilan. Beberapa serangan proyektil Sulit atau Mustahil untuk dihindari (lihat Menghindari Senjata Proyektil).

### Mengemudi (berbagai)

**Peluang Dasar:** 20% atau 01% (lihat di bawah)

**Kategori:** Manipulasi

Gunakan Mengemudi untuk mengarahkan, bermanuver, dan mengendalikan kendaraan darat, baik yang ditarik oleh hewan maupun yang digerakkan oleh mesin. Untuk kendaraan umum, peluang dasarnya adalah 20%, untuk kendaraan yang tidak diketahui/tidak umum, itu 01%. Kegagalan Mutlak saat mengemudi secara normal berarti kendaraan mungkin tidak mau hidup, atau tabrakan atau kecelakaan. Keberhasilan berarti karakter Anda sampai ke tempat tujuan, atau mampu memperbaiki kondisi buruk (seperti selip atau berputar), dan keberhasilan yang unggul berarti jarak lebih jauh ditempuh dalam waktu lebih singkat, bahan bakar lebih sedikit yang digunakan, atau manfaat lainnya.

**Spesialisasi:** Mobil, Sepeda, Kereta, Gerobak, Kereta Perang, Kereta Anjing, Sepeda Motor, Truk, Kereta Luncur, dll.

**Catatan Latar:** Sesuaikan peluang dasar dengan keakraban pengguna sebagaimana mestinya, tetapi jika ragu, asumsikan bahwa prinsip dasar berlaku.

**Catatan Sistem:** Sebagian besar mengemudi sehari-hari adalah tugas Otomatis—lemparan Mengemudi hanya dalam kondisi buruk atau untuk mengemudi luar biasa (mengebut melalui area pusat kota yang sibuk, mengejar kendaraan lain, dll.). Untuk membuntuti kendaraan lain tanpa diketahui, gunakan lemparan Mengemudi yang berlawanan vs. Melihat pengemudi lain. Untuk menyelesaikan pengejaran kendaraan, lihat Pengejaran.

### Senjata Energi (berbagai)

**Peluang Dasar:** Sesuai spesialisasi senjata

**Kategori:** Tempur

Gunakan ini untuk menembakkan segala jenis senjata energi terarah atau senjata sinar genggam (laser, blaster, disruptor, dll.).

**Spesialisasi:** Pistol Energi, Senapan Energi, dll. Spesialisasi setara dengan jenis. Lihat **Kelas Senjata** di *Bab Delapan: Peralatan* untuk jenis senjata api.

**Efek:** Lihat **Matriks Serangan dan Pertahanan** dan **Tabel Kegagalan Mutlak Senjata Proyektil** untuk efek spesifik. Senjata energi biasanya menusuk.

**Catatan Latar:** Senjata energi biasanya hanya muncul di latar futuristik. Siapa pun dari budaya tanpa senjata tersebut harus melakukan lemparan Ide Sulit untuk mengetahui cara menggunakannya, dan mungkin hanya setelah melihatnya digunakan. Kegagalan atau Kegagalan Mutlak bisa menjadi bencana, bagi senjata atau penggunanya! Latar belakang militer mungkin diperlukan untuk mempelajari keterampilan ini.

**Catatan Sistem:** Beberapa senjata energi memiliki persyaratan STR dan DEX minimum untuk menggunakan senjata tersebut, sesuai tabel senjata di *Bab Delapan: Peralatan*. Senjata energi tidak menggunakan pengubah kerusakan tetapi dapat digunakan untuk serangan terarah. Mereka tidak dapat dihindari, meskipun pergerakan dan orientasi target dapat memaksakan penalti pada tembakan, seperti yang dijelaskan dalam Menghindari Senjata Proyektil. Memblokir tembakan energi dengan keterampilan Perisai adalah Sulit.

### Etiket (berbagai)

**Peluang Dasar:** 05%

**Kategori:** Komunikasi

Sebagian besar budaya, kasta, atau kelompok memiliki aturan sosial dan kode perilaku. Pakaian yang tepat, cara bicara, perilaku spesifik, bahasa tubuh, atau bahkan pengadilan dengan pertempuran semuanya dapat berperan dalam etiket suatu kelompok. Gunakan keterampilan ini setiap kali karakter Anda perlu berperilaku dengan tepat, baik menghadiri pesta makan malam ala Victoria, wawancara kerja penting, atau bergabung dengan keluarga kejahatan terorganisir. Lemparan etiket mungkin mendapatkan audiensi dengan karakter non-pemain penting atau mengesankan orang lain dengan pengetahuan Anda tentang adat istiadat sosial mereka. Kegagalan Mutlak mewakili faux pas yang membawa bencana, gagal tidak mengesankan, dan keberhasilan atau lebih menghasilkan hasil yang unggul dan meningkatkan penerimaan seseorang.

**Spesialisasi**: Berdasarkan kelompok. Untuk mempelajari tata krama kelompok kedua, pelajari keterampilan Etiket kedua dengan spesialisasi tersebut.

**Catatan Sistem:** Gamemaster hanya boleh meminta lemparan Etiket dalam keadaan penting: misalnya, pertama kali berinteraksi dengan kelompok penting, acara penting, atau saat mencoba meningkatkan status dalam kelompok.

### Bicara Cepat

**Peluang Dasar:** 05%

**Kategori:** Komunikasi

Senjata pilihan salesman, mata-mata, atau penipu, gunakan keterampilan ini untuk meyakinkan target dengan cepat tentang sesuatu yang mungkin tidak mereka percayai, atau meyakinkan mereka untuk membuat keputusan cepat yang bertentangan dengan penilaian mereka yang lebih baik. Gunakan ini untuk menggertak melewati penjaga pintu atau membuat penjualan sulit berjalan cepat. Bicara Cepat cepat digunakan, dan biasanya sementara: beberapa menit kemudian dan lemparan Ide yang berhasil, target mungkin sadar kembali. Untuk benar-benar mengubah pikiran seseorang, gunakan keterampilan Menawar atau Membujuk. Kegagalan Mutlak biasanya menarik permusuhan, gagal tidak membawa Anda ke mana-mana, dan keberhasilan atau lebih memiliki efektivitas yang lebih baik, atau bahkan meyakinkan seseorang secara permanen tentang posisi Anda.

**Catatan Sistem:** Roleplaying yang baik dan/atau argumen yang efektif mungkin membuat upaya Bicara Cepat menjadi Mudah, sama seperti roleplaying yang buruk dan/atau argumen yang lemah mungkin membuatnya Sulit. Mencoba Bicara Cepat pada target setelah upaya pertama di Menawar, Bicara Cepat, atau Membujuk gagal adalah Sulit.

### Manipulasi Halus

**Peluang Dasar:** 05%

**Kategori:** Manipulasi

Ukuran kemantapan dan koordinasi tangan-mata, keterampilan ini mencakup penggunaan tangan dan jari dengan hati-hati. Gunakan saat membuka kunci, menjinakkan jebakan, mengikat atau melepas simpul, dan membongkar perangkat mekanis. Kegagalan Mutlak mengacaukan segalanya, mungkin merusak atau membuat macet barang yang dimanipulasi. Keberhasilan yang unggul membutuhkan waktu lebih sedikit, membuatnya mudah di masa depan, tidak menunjukkan tanda-tanda gangguan, atau manfaat serupa lainnya.

**Catatan Sistem:** Menggunakan keterampilan ini tanpa alat yang sesuai, jika diperlukan, adalah Sulit.

### Senjata Api (berbagai)

**Peluang Dasar:** Sesuai spesialisasi senjata

**Kategori:** Tempur

Keterampilan ini mengukur akurasi dengan senjata api, dan memberikan keakraban dengan penanganan, pemeliharaan, dan perbaikan dasar.

**Spesialisasi:** Senapan Mesin, Pistol, Revolver, Senapan, Senapan Patah, Senapan Submesin. Spesialisasi setara dengan jenis. Lihat **Kelas Senjata** di *Bab Delapan: Peralatan* untuk jenis senjata api.

**Efek:** Lihat **Matriks Serangan dan Pertahanan** dan **Tabel Kegagalan Mutlak Senjata Proyektil** untuk efek spesifik. Senjata api biasanya menusuk (lihat *Bab Delapan: Peralatan* untuk detail).

**Catatan Latar:** Karakter mana pun yang tidak terbiasa dengan senjata api harus melakukan lemparan Ide Sulit untuk mengetahui cara menggunakannya jika mereka belum pernah melihatnya digunakan. Kegagalan atau Kegagalan Mutlak bisa menjadi bencana, bagi senjata atau penggunanya!

**Catatan Sistem:** Beberapa senjata api memiliki persyaratan STR dan DEX minimum, sesuai tabel senjata di *Bab Delapan: Peralatan*. Senjata api tidak menggunakan pengubah kerusakan, dan dapat digunakan untuk serangan terarah.

### Pertolongan Pertama

**Peluang Dasar:** 30%

**Kategori:** Mental

Gunakan keterampilan ini untuk menghentikan pendarahan, membalut luka dan mengobati luka bakar ringan, memasang anggota tubuh yang patah, menyadarkan seseorang yang tenggelam, atau menyadarkan orang yang pingsan. Ini mencakup triase dan perawatan medis dasar tetapi memiliki sedikit efek pada racun, penyakit, atau penyakit fisik halus: ini harus diobati dengan keterampilan Pengobatan, meskipun gamemaster Anda mungkin mengizinkan lemparan Sulit untuk perawatan darurat racun atau penyakit. Pertolongan Pertama membutuhkan satu putaran tempur penuh dan biasanya tidak dapat dilakukan saat dalam pertempuran. Jika terputus, upaya harus dimulai kembali.

**Efek:** Pertolongan Pertama memiliki efek spesifik:

* **Kegagalan Mutlak:** Pasien menerima 1 poin hit kerusakan umum, dan kondisi mereka tetap tidak berubah.
* **Kegagalan:** Tidak ada efek, dan tidak ada upaya Pertolongan Pertama lebih lanjut yang dapat dilakukan.
* **Keberhasilan:** Salah satu dari berikut ini (pilihan pemain):
  * Menyembuhkan 1D3 poin hit untuk satu luka atau cedera.
  * Karakter pada 0 atau poin hit negatif di putaran ini atau sebelumnya, dapat dihidupkan kembali jika total poin hit mereka dibawa ke 1+.
  * Menyadarkan karakter yang pingsan.
  * Menghentikan kerusakan pendarahan dari keberhasilan khusus pendarahan atau kerusakan berkelanjutan lainnya ke lokasi hit (lihat **Poin Hit per Lokasi Hit**).
* **Khusus:** Seperti di atas tetapi menyembuhkan 2D3 poin hit.
* **Kritis:** Seperti di atas tetapi menyembuhkan 3+1D3 poin hit.

**Catatan Sistem:** Lacak cedera individu, karena Pertolongan Pertama dapat digunakan pada setiap cedera, berpotensi menyembuhkannya hingga jumlah poin hit yang ditimbulkan cedera tersebut. Hanya satu upaya yang dapat dilakukan per luka. Kondisi berbahaya atau tidak sehat dapat membuat lemparan menjadi Sulit. Karakter Anda dapat menambahkan 1/2 dari peringkat keterampilan Pengobatan mereka dan 1/5 dari peringkat keterampilan Sains (Farmasi) mereka sebagai bonus sementara untuk lemparan Pertolongan Pertama. Peralatan medis modern (obat penghilang rasa sakit, perban, atau obat-obatan) dapat menambahkan bonus hingga +20% pada peringkat keterampilan, sementara teknologi medis futuristik (semprotan kulit buatan, pemindai medis, penyembuh nanotech) dapat membuat semua lemparan Pertolongan Pertama menjadi Mudah.

### Terbang

**Peluang Dasar:** DEX×4 atau 1/2 DEX (lihat di bawah)

**Kategori:** Fisik

Untuk makhluk dengan sayap, perlengkapan yang sesuai (sabuk antigravitasi, sepatu roket, dll.), atau kekuatan terbang, gunakan keterampilan ini untuk bermanuver di udara tanpa kendaraan. Gunakan Terbang untuk melakukan manuver udara atau akrobat tanpa kehilangan kendali, mengungguli musuh dalam pertempuran udara, atau terbang dalam kondisi cuaca buruk. Peluang dasar adalah DEX×4 untuk makhluk bersayap; 1/2 DEX untuk semua yang lain. Untuk paket jet, gantungan layang, dll. gunakan keterampilan Pilot. Lihat **Pertempuran Udara** dan **Pengejaran** untuk informasi lebih lanjut.

**Efek:** Terbang memiliki efek spesifik:

* **Kegagalan Mutlak:** Berhenti dan mungkin bahkan jatuh. Karakter bersayap harus melakukan lemparan Stamina yang berhasil atau menderita 1D3–1 poin kerusakan dari keseleo atau regangan. Perangkat berhenti atau gagal dan tidak dapat menjaga karakter Anda tetap tinggi. Penerbang jatuh satu putaran per tingkat ketinggian di atas tanah. Lakukan lemparan Terbang Sulit untuk tetap tinggi—gagal berarti kecelakaan (lihat **Jatuh**).
* **Kegagalan:** Keluar jalur atau turun ke ketinggian yang tidak diinginkan. Tidak pergi ke mana-mana.
* **Keberhasilan:** Menyelesaikan manuver yang diinginkan atau menahan posisi dalam pengejaran.
* **Khusus:** Menyelesaikan manuver yang diinginkan dan mendapatkan bonus +10% untuk lemparan Menghindar, Terbang, atau serangan berikutnya yang dibuat dalam pertempuran udara.
* **Kritis:** Menyelesaikan manuver yang diinginkan dan mendapatkan bonus +25% untuk lemparan Menghindar, Terbang, atau serangan berikutnya yang dibuat dalam pertempuran udara.

**Catatan Sistem:** Penerbangan singkat yang tidak berkesan dalam kondisi normal atau sedang adalah Otomatis. Lihat **Kondisi Cuaca** untuk pengubah pada penerbangan. Lemparan Stamina diperlukan untuk penerbangan jarak jauh, dengan pengganda karakteristik berkurang satu setiap empat jam atau sebagian darinya yang dihabiskan di udara. Penerbangan normal tidak mengeluarkan poin kelelahan, sementara manuver atau penerbangan yang menekan adalah aktivitas yang berat. Terbang saat mengenakan Zirah atau perlengkapan berat dikenakan penalti yang setara dengan total ENC karakter Anda saat ini (lihat **Encumbrance**). Karakter terbang yang membawa benda berat harus mencocokkan STR mereka terhadap SIZ benda yang dibawa pada tabel resistensi untuk tetap tinggi.

### Permainan

**Peluang Dasar:** INT+POW%

**Kategori:** Mental

Gunakan keterampilan ini saat memainkan permainan, dari catur atau backgammon hingga kontes judi yang melibatkan kartu, dadu, atau elemen acak atau strategis lainnya. Penjudi mencari nafkah menggunakan keterampilan ini, tetapi bagi kebanyakan orang lain itu hanya sumber hiburan. Kegagalan Mutlak membersihkan Anda dan berpotensi membuat Anda berutang, gagal berarti Anda kalah dalam putaran, keberhasilan menghasilkan kemenangan kecil, dan keberhasilan yang unggul menuai manfaat yang lebih besar dan mungkin bahkan meningkatkan status seseorang.

**Catatan Sistem:** Gunakan ini untuk menyelesaikan permainan di mana keterampilan dan strategi memainkan peran tertentu, dengan pemain lain melakukan lemparan yang berlawanan. Untuk curang, gunakan Kecekatan Tangan atau Bicara Cepat, yang berlawanan dengan Mengamati atau Wawasan. Hasil yang sepenuhnya acak menggunakan lemparan Keberuntungan.

### Bergulat

**Peluang Dasar:** 25%

**Kategori:** Tempur

Gunakan keterampilan ini untuk gulat atau pertarungan apa pun di mana tujuannya adalah untuk menaklukkan target tanpa menyebabkan bahaya. Biasanya ini tidak mematikan, tetapi dapat digunakan untuk memberikan bahaya nyata.

**Efek:** Bergulat memiliki efek spesifik:

* **Kegagalan Mutlak:** Lempar pada tabel **Kegagalan Mutlak Senjata Alami**.
* **Kegagalan:** Target menghindari ditahan, atau upaya pelarian gagal.
* **Keberhasilan:** Target digulat, penyerang dapat menerapkan satu efek gulat (lihat **Efek Bergulat** di bawah) putaran berikutnya jika pegangan dipertahankan.
* **Khusus:** Penyerang secara otomatis berhasil dalam lemparan Bergulat berikutnya untuk mempertahankan pegangan dan mendapatkan bonus karakteristik +3 pada lemparan resistensi apa pun untuk efek gulat yang dibuat putaran ini.
* **Kritis:** Penyerang secara otomatis berhasil dalam lemparan Bergulat berikutnya untuk mempertahankan pegangan, dan karakteristik target dibagi dua dalam lemparan resistensi apa pun untuk efek gulat selama sisa putaran tempur itu.

**Catatan Sistem:** Penyerang harus berhasil melakukan Bergulat untuk mendapatkan pegangan pada target. Serangan yang berhasil meraih lokasi hit acak (jika sistem itu digunakan). Gulat dapat ditangkis dengan keterampilan Bergulat: jika bek menangkis dengan senjata atau perisai, penyerang secara otomatis meraih lengan senjata atau lengan perisai jika lemparan Bergulat berhasil. Setelah pegangan ditetapkan, penyerang harus melakukan lemparan Bergulat yang berhasil setiap putaran untuk mempertahankan pegangan. Melakukan tindakan apa pun selain salah satu efek gulat memutus pegangan. Setelah ditahan, target dapat menyerang penyerang mereka jika mereka memiliki anggota tubuh yang bebas, menggunakan Berkelahi atau senjata kecil apa pun (pisau atau pistol). Jika kedua tangan bebas, target dapat mencoba Bergulat balik.

Beberapa penyerang dapat mencoba bergulat dengan satu target, meskipun tidak lebih dari dua penyerang yang dapat memegang satu lokasi. Gabungkan peringkat STR kedua penyerang dalam semua tes yang diperebutkan.

> ### Efek Bergulat
>
> Penyerang dengan pegangan pada target mereka dapat mencoba salah satu dari efek berikut, satu per putaran, dimulai putaran setelah Bergulat ditahan. Beberapa efek hanya dapat digunakan jika penyerang telah meraih bagian tubuh tertentu (lihat di bawah). Karena beberapa efek melibatkan lokasi hit tertentu, gamemaster mungkin perlu menafsirkan hasilnya.
>
> * **Ubah Pegangan (Apa saja):** Cocokkan DEX vs. DEX pada tabel resistensi. Jika berhasil, penyerang mengalihkan pegangan ke lokasi hit pilihan yang berbeda. Jika tidak berhasil, target dapat melakukan tindakan, meskipun penyerang masih memiliki pegangan pada lokasi asli.
>
> * **Lucuti Target (Lengan):** Penyerang mencocokkan STR+SIZ vs. STR target pada tabel resistensi. Jika penyerang berhasil, target menjatuhkan apa pun di tangan itu, dan lengan tetap tidak bergerak. Jika tidak berhasil, target tetap memegang barang(-barang) yang ditahan, meskipun lengan mereka tetap tidak bergerak. Penyerang mempertahankan pegangan.
>
> * **Raih Lokasi Kedua (Lengan/Kaki)**: Penyerang mencoba meraih dan menahan lokasi hit lain pilihan mereka. Cocokkan DEX penyerang vs. DEX+SIZ target pada tabel resistensi. Jika penyerang menang, mereka meraih lokasi kedua dan menjaga anggota tubuh yang sudah ditahan tetap terjebak. Jika tidak berhasil, target segera mencocokkan STR mereka vs. STR penyerang pada tabel resistensi untuk memutuskan semua pegangan. Penyerang dapat memegang lokasi sebanyak tangan yang mereka miliki.
>
> * **Imobilisasi Anggota Tubuh (Lengan/Kaki)**: Cocokkan STR penyerang vs. STR target pada tabel resistensi. Jika penyerang menang, tidak ada tindakan yang dapat dilakukan dengan anggota tubuh itu sampai pegangan diputus. Jika tidak berhasil, target dapat melakukan tindakan, meskipun penyerang masih memiliki pegangan pada anggota tubuh tersebut.
>
> * **Cederai Target (Kepala/Dada/Perut)**: Penyerang menyikut, mencungkil, atau meremas, menimbulkan 1D3 poin kerusakan ditambah pengubah kerusakan mereka pada target (umum atau ke lokasi hit yang ditahan). Target dapat segera mencocokkan STR mereka vs. STR penyerang pada tabel resistensi untuk memutus pegangan.
>
> * **Imobilisasi Target (Kepala/Dada/Perut)**: Cocokkan STR penyerang vs. STR target pada tabel resistensi. Jika penyerang menang, target tidak dapat melakukan tindakan sama sekali sampai pegangan diputus. Jika tidak berhasil, target masih dapat melakukan tindakan, meskipun penyerang masih memiliki pegangan.
>
> * **Jatuhkan Target (Kepala/Dada/Perut):** Penyerang secara otomatis menindih target, menyeret kedua karakter tengkurap. Target harus melakukan lemparan Kelincahan atau menderita 1 poin kerusakan umum (atau ke lokasi hit acak).
>
> * **Cekik Target (Kepala):** Target harus melakukan lemparan CON×1% setiap putaran atau menderita 1D3 poin hit kerusakan pada poin hit umum mereka. Target dapat segera mencocokkan STR mereka vs. STR karakter penyerang pada tabel resistensi untuk memutus pegangan. Setelah penyerang mulai mencekik, mereka tidak perlu melakukan lemparan Bergulat lebih lanjut setiap putaran untuk mempertahankan pegangan. Target harus mencoba melepaskan diri (lihat **Tersedak, Tenggelam, dan Asfiksia**).
>
> * **Lempar Target (Khusus):** Cocokkan STR+DEX penyerang terhadap SIZ+DEX target pada tabel resistensi. Melempar target melepaskan pegangan apa pun. Jika penyerang berhasil, target dilempar 1D6 meter, secara otomatis dijatuhkan, dan harus melakukan lemparan Kelincahan atau menderita 1D6 poin kerusakan umum (atau ke lokasi hit acak). Lemparan DEX×1% memungkinkan mereka mendarat di kaki mereka tanpa mengalami kerusakan. Jika lemparan tidak berhasil, target tidak dilempar dan dapat melakukan tindakan, meskipun penyerang mempertahankan pegangan. Lemparan dapat dicoba dengan anggota tubuh mana pun yang ditahan, tetapi keberhasilan lemparan dengan kepala hanya menimbulkan 1D6 poin kerusakan umum (atau ke lokasi hit kepala) dan tidak menghasilkan lemparan.

### Mesin Berat (berbagai)

**Peluang Dasar:** 01%

**Kategori:** Manipulasi

Gunakan keterampilan ini untuk mengoperasikan mesin apa pun yang lebih besar dari orang dan kendaraan beroda apa pun yang lebih berat dari van pindahan, seperti forklift, tank, buldoser, derek, lokomotif, atau mengoperasikan mesin di kapal laut besar. Mesin Berat juga mencakup perawatan dasar dan pemeliharaan preventif, meskipun setelah mesin berat gagal, keterampilan Perbaikan diperlukan untuk memperbaikinya. Untuk pesawat, gunakan keterampilan Pilot. Kegagalan Mutlak menghasilkan kecelakaan, masalah mesin, dan/atau kerusakan properti, sementara gagal berarti mesin tidak melakukan tugas yang diinginkan. Keberhasilan dan keberhasilan yang unggul menunjukkan efektivitas yang lebih besar.

**Spesialisasi:** Berdasarkan jenis kendaraan (Kendaraan Lapis Baja, Boiler, Buldoser, Derek, Mesin, Turbin, dll.).

**Catatan Sistem:** Umumnya, mesin berat tidak dapat berpartisipasi dalam pengejaran, tetapi jenis mesin tertentu dapat digunakan sebagai senjata, melakukan kerusakan 3D6 hingga 6D6, berdasarkan ukuran perangkat. Menggunakan keterampilan ini di luar kinerja normal adalah Sulit, seperti halnya operasi dan pemeliharaan dalam kondisi ekstrem.

### Senjata Berat (berbagai)

**Peluang Dasar:** Sesuai spesialisasi senjata

**Kategori:** Tempur

Gunakan keterampilan ini untuk mengoperasikan senjata peledak atau senjata tembakan langsung besar yang mendorong batas portabilitas, seperti bazoka, peluncur roket, senapan mesin berat, meriam laser yang dipasang di tripod, dan lain-lain. Gunakan keterampilan Artileri untuk persenjataan yang lebih berat seperti meriam, mesin pengepungan, atau senjata yang dipasang di kendaraan atau pintle.

**Spesialisasi:** Senjata berat berada di antara senjata tradisional dan artileri dan sering digunakan untuk mengirimkan bahan peledak. Spesialisasi berdasarkan jenis senjata, dan termasuk Bazoka, Penyembur Api, Senapan Mesin Mini, Peluncur Roket, dll. Lihat **Artileri dan Bahan Peledak** di *Bab Delapan: Peralatan* untuk detail lebih lanjut.

**Efek:** Lihat **Matriks Serangan dan Pertahanan** dan **Tabel Kegagalan Mutlak Senjata Proyektil** untuk efek spesifik. Senjata berat biasanya menusuk atau knockback.

**Catatan Sistem:** Sebagian besar senjata berat menggunakan tembakan otomatis, atau menembakkan persenjataan peledak. Lihat **Tembakan Otomatis** dan **Ledakan**. Semua tembakan yang meleset pergi ke suatu tempat; gamemaster harus memutuskan di mana tembakan peledak yang meleset berdampak.

### Bersembunyi

**Peluang Dasar:** 10%

**Kategori:** Fisik

Seni menyembunyikan objek atau diri sendiri dari pandangan. Karakter Anda harus menggunakan keterampilan ini untuk melarikan diri saat pengejaran, menjaga objek agar mudah tidak terdeteksi, atau menghindari pengawasan. Kegagalan Mutlak menarik perhatian segera, gagal berarti lemparan Melihat untuk menemukan penyembunyi menjadi *Mudah*, keberhasilan memerlukan lemparan Melihat untuk ditemukan, dan keberhasilan yang unggul membuat lemparan Melihat menjadi *Sulit* atau *Mustahil*.

**Catatan Sistem:** Pemeriksaan Bersembunyi dilawan oleh Melihat pengamat atau pencari. Penutup yang memadai sangat penting; bersembunyi di penutup yang minim adalah Sulit, dan kurangnya penutup sama sekali (terbuka lebar, siang hari bolong) membuatnya Mustahil. Pakaian atau perlengkapan khusus (cat wajah, pakaian kamuflase, dll.) dapat menambahkan bonus hingga +20%, tergantung pada kualitasnya. Kondisi lingkungan mungkin menawarkan bonus tambahan. Seseorang yang bersembunyi harus diam sebisa mungkin; bergerak saat menggunakan Bersembunyi adalah Sulit, dan bergerak lebih dari satu meter dalam satu putaran tempur memerlukan lemparan Bersembunyi dan Siluman. Bersembunyi hanya efektif terhadap deteksi visual dan pendengaran; sensor canggih mungkin menemukan karakter yang bersembunyi dengan panas tubuh, atau hidung yang tajam mungkin menemukan mereka menggunakan keterampilan Perasaan.

### Wawasan

**Peluang Dasar:** 05%

**Kategori:** Persepsi

Gunakan keterampilan ini untuk mengevaluasi karakter, kondisi emosional, dan motif orang lain berdasarkan bahasa tubuh, pola bicara, dan faktor tak berwujud lainnya. Wawasan memungkinkan penjudi merasakan gertakan, atau detektif mengendus kebohongan. Kegagalan Mutlak memberikan informasi yang menyesatkan, gagal tidak menghasilkan apa pun, keberhasilan memungkinkan Anda mengetahui apa yang mungkin dipikirkan target, dan keberhasilan yang unggul mengungkapkan kepada Anda pikiran terdalam mereka (dalam batas wajar).

**Catatan Sistem:** Keterampilan ini dapat dilawan oleh Etiket, Bicara Cepat, atau bahkan Membujuk. Menggunakan keterampilan ini pada teman dekat, anggota keluarga, atau pasangan adalah Mudah. Jika menggunakan sistem **Sifat Kepribadian**, Wawasan dapat menentukan kekuatan sifat kepribadian tersebut dalam istilah umum (seperti "Sangat egois", misalnya).

### Melompat

**Peluang Dasar:** 25%

**Kategori:** Fisik

Gunakan keterampilan ini untuk melompat demi ketinggian atau jarak, melompati rintangan. Dalam kondisi normal, manusia yang cukup bugar dapat melompat setinggi tingginya secara horizontal dan setengah tingginya secara vertikal dari posisi berdiri, dan kira-kira tiga meter secara horizontal dengan awalan lari. Kegagalan Mutlak menghasilkan jatuh (1D6 poin hit kerusakan umum atau ke lokasi hit), keberhasilan menghasilkan lompatan seperti yang dijelaskan di atas, keberhasilan khusus meningkatkan jumlah sebesar 50%, dan keberhasilan kritis menggandakannya. Melompat juga dapat mengurangi kerusakan jatuh sebesar 1D6 per tingkat keberhasilan (keberhasilan normal sebesar –1D6, khusus sebesar –2D6, kritis sebesar –3D6).

**Catatan Sistem:** Lompatan berdiri memerlukan 1 pangkat DEX. Lompatan lari menggunakan pangkat DEX yang sama dengan pergerakan lari normal. Melakukan lompatan yang mencolok atau pamer (jungkir balik, salto, dll.) adalah Sulit.

### Pengetahuan (berbagai)

**Peluang Dasar:** 05% atau 00% (lihat di bawah)

**Kategori:** Mental

Keterampilan ini mewakili pengalaman atau studi akademik cabang pengetahuan. Gunakan keterampilan Pengetahuan untuk mengingat fakta-fakta yang relevan, untuk menganalisis atau mengidentifikasi petunjuk, membuat anggapan logis, dan mengingat trivia yang mengesankan. Untuk yang lebih ilmiah, lihat Sains. Kegagalan Mutlak menghasilkan kebohongan yang menyesatkan dan tidak akurat, dan keberhasilan menghasilkan informasi yang lebih berguna dan mendetail.

**Spesialisasi:** Akuntansi, Antropologi, Arkeologi, Sejarah, Hukum, Okultisme, Agama, dll. (lihat daftar terdekat) Gamemaster harus menentukan apakah keterampilan Pengetahuan memiliki peluang dasar 05% untuk spesialisasi yang umum, atau 00% untuk yang memerlukan penelitian dan studi untuk memiliki pemahaman dasar.

**Catatan Latar:** Dalam latar fantasi dan sejarah, hanya sebagian kecil spesialisasi yang ada. Seiring berkembangnya latar yang lebih modern, spesialisasi meningkat.

**Catatan Sistem:** Karakteristik EDU opsional dan lemparan Tahu mencakup sebagian besar pengetahuan umum—informasi khusus diwakili oleh spesialisasi keterampilan Pengetahuan yang sesuai.

> ### Spesialisasi Pengetahuan
>
> Spesialisasi ini tidak lengkap dan dapat diperluas sesuai kebutuhan. Beberapa spesialisasi memiliki sub-spesialisasi, fokus spesifik pada satu subjek dalam bidang tersebut. Tulis seperti ini: Pengetahuan (Kelompok/Templar) atau Pengetahuan (Templar), sesuai keinginan.
>
> **Pengetahuan Akademik (Hanya Era Kuno):** Di era ketika seluruh pengetahuan tertulis dapat dimuat dalam satu perpustakaan, ini mewakili seberapa 'terpelajar' seseorang. Gunakan seperti lemparan Tahu untuk hampir semua bidang pengetahuan.
>
> **Akuntansi:** Gunakan untuk menganalisis pembukuan, analisis keuangan (utang, kebangkrutan, kekayaan, kekayaan bersih, dll.), dan untuk mengidentifikasi ketidakberaturan keuangan (pengeluaran aneh, penggelapan, atau rejeki nomplok yang aneh).
>
> **Antropologi:** Studi tentang budaya, termasuk adat istiadat, seni, dan perilaku. Gunakan untuk mengidentifikasi budaya asli seseorang atau artefak atau untuk menentukan adat istiadat sosial dan perilaku budaya.
>
> **Arkeologi:** Studi tentang budaya kuno dan yang telah punah. Gunakan untuk mengidentifikasi artefak dari budaya yang mati, melihat barang palsu, atau melakukan penggalian dan membuat kesimpulan yang masuk akal tentang sejarah situs atau wilayah.
>
> **Sejarah Seni:** Studi tentang seni dari berbagai era dan tempat, termasuk gaya, seniman terkenal, dan karya spesifik. Gunakan untuk mengidentifikasi asal usul karya seni, melihat barang palsu, atau menghubungkan karya dengan master yang dikenal.
>
> **Pengetahuan Menghujat:** Studi tentang rahasia okultisme yang mengerikan dan kejahatan kuno. Semakin banyak pengetahuan ini dimiliki seseorang, semakin sedikit cengkeraman mereka pada kewarasan (lihat Kewarasan). Keterampilan Pengetahuan Menghujat dimulai pada 0%, bukan 05%, dan tidak dapat ditingkatkan dengan pengalaman.
>
> **Bisnis:** Studi tentang praktik kewirausahaan, peraturan serikat, hukum korporat, dan teknik manajemen untuk perusahaan komersial. Gunakan untuk mengidentifikasi pemain utama di bidang atau pasar, memprediksi tren ekonomi, dan mendapatkan keuntungan.
>
> **Mata-mata:** Pengetahuan tentang organisasi mata-mata, mata-mata dan agen terkenal atau terkenal; metodologi, tokoh terkemuka, organisasi, taktik, dll. Gunakan untuk mengetahui tentang agensi dan modus operandi mereka. Pemecahan kode ditangani dengan Sains (Kriptografi).
>
> **Cerita Rakyat:** Pengetahuan tentang adat istiadat, cerita, lirik, kisah tinggi, dan legenda suatu orang atau wilayah tertentu. Gunakan keterampilan ini untuk mengidentifikasi atau mengingat elemen cerita rakyat atau dongeng, baik pedesaan maupun supranatural.
>
> **Kelompok/Sub-spesialisasi:** Informasi umum tentang budaya, kebangsaan, kelompok, klub, atau kelas sosial tertentu. Gunakan keterampilan ini untuk mengidentifikasi anggota kelompok tersebut, mengetahui 'siapa siapa' dalam kelompok tersebut, dan mengingat adat istiadat sosial dan adat istiadat kelompok tersebut.
>
> **Sejarah:** Studi tentang peristiwa dan era masa lalu. Gunakan keterampilan ini untuk mengingat pentingnya tempat, orang, peristiwa, atau objek apa pun, atau untuk menebak dengan benar kapan suatu objek atau dokumen dibuat.
>
> **Hukum:** Studi tentang praktik dan preseden hukum. Gunakan keterampilan ini untuk menilai legalitas tindakan yang diusulkan, menemukan preseden hukum, membebaskan diri dari masalah dengan pihak berwenang, atau membangun kasus untuk atau melawan penjahat.
>
> **Linguistik:** Studi tentang bahasa itu sendiri, dari dasar tata bahasa dan kosakata hingga bagaimana masyarakat membentuk bahasa. Gunakan keterampilan ini untuk mengidentifikasi bahasa yang tidak dikenal atau memecahkan beberapa kata atau memahami makna dasar, meskipun keterampilan ini bukan pengganti keterampilan Bahasa yang sesuai.
>
> **Sastra**: Studi tentang prosa, puisi, dan esai yang signifikan, biasanya dari satu budaya. Gunakan keterampilan ini untuk mengidentifikasi kutipan atau kiasan ke karya tertentu dengan benar, mengidentifikasi kapan dan di mana fragmen yang tidak dikenal mungkin ditulis, menghubungkan karya misterius dengan penulis yang dikenal, melihat karya sastra palsu, atau mengutip bagian yang sangat tepat dari karya sastra terkenal.
>
> **Dunia Alam**: Pengetahuan tentang lingkungan, pola cuaca, flora, fauna, dan sejarah alam suatu daerah, terutama dari pengertian praktis daripada latar belakang ilmiah.
>
> **Okultisme:** Pengetahuan tentang sihir, legenda yang tidak jelas, dan apa yang disebut 'sejarah rahasia' yang mencakup segalanya mulai dari filosofi rahasia hingga teori konspirasi modern. Gunakan keterampilan ini untuk menyadari pentingnya simbol alkimia atau astrologi, mengidentifikasi tradisi magis mana yang menciptakan ruang kerja ritual, mengetahui nama roh yang kuat, atau mengidentifikasi faksi rahasia yang berkonspirasi untuk memanipulasi situasi.
>
> **Politik:** Studi dan pengetahuan tentang sistem politik atau cara pemerintahan lain yang dipaksakan atau dipilih oleh warga negara. Gunakan keterampilan ini untuk memahami blok kekuatan utama di suatu daerah, cara memengaruhi struktur politik, dan cara warga akan bereaksi terhadap perubahan atau pengaruh politik.
>
> **Wilayah/Sub-spesialisasi:** Informasi umum tentang kota, wilayah, atau negara tertentu, termasuk orang penting, landmark lokal, dan sejarah regional. Gunakan keterampilan ini untuk menemukan penginapan murah di kota yang sibuk, mengingat penginapan pinggir jalan terbaik di provinsi tersebut, memahami politik lokal, atau menemukan pedagang atau pedagang tertentu.
>
> **Agama/Sub-spesialisasi:** Detail mitos, liturgi, dan praktik agama tertentu. Gunakan keterampilan ini untuk menghindari pelanggaran tabu, mengingat hukum dan kewajiban keyakinan, berhasil memperdebatkan poin teologi, mengidentifikasi regalia atau teks agama, memilah ortodoksi dari bidah, atau memimpin upacara keagamaan dengan benar.
>
> **Jalanan:** Pengetahuan tentang segmen masyarakat yang lebih teduh, dari protokol orang miskin hingga cara dunia bawah kriminal. Gunakan keterampilan ini untuk menemukan rumah aman, mengidentifikasi pagar untuk barang curian, atau mengetahui siapa yang harus dimintai bantuan semi-legal atau ilegal lainnya.

### Bahasa (berbagai)

**Peluang Dasar:** INT×5 (atau EDUx5) atau 00% (lihat di bawah)

**Kategori:** Komunikasi

Kemampuan untuk berkomunikasi secara verbal dalam suatu bahasa, baik bahasa ibu seseorang maupun bahasa lain. Bahasa lain dapat dipelajari di rumah, sekolah, gereja, tempat kerja, atau di jalanan, tergantung pada budaya dan pola asuh. Sebagian besar karakter memulai dengan mengetahui bahasa mereka sendiri pada INT×5. Karakter bilingual harus menghabiskan poin keterampilan untuk keterampilan bahasa lain. Percakapan sehari-hari dalam Bahasa sendiri adalah Otomatis dan tidak memerlukan lemparan keterampilan. Lemparan keterampilan hanya diperlukan saat mengomunikasikan konsep yang sulit atau dalam situasi yang sulit dan tidak biasa, seperti menerjemahkan. Keterampilan ini mencakup komunikasi verbal—membaca dan menulis menggunakan Literasi. Kegagalan Mutlak berarti kesalahpahaman yang membawa bencana, sementara keberhasilan menghasilkan komunikasi yang lancar dan terkadang fasih.

**Spesialisasi:** Per bahasa. Setiap spesialisasi berlaku untuk satu bahasa; jika Anda ingin karakter Anda mempelajari bahasa tambahan, mereka harus membangun keterampilan Bahasa tambahan.

**Catatan Sistem:** Jika aturan opsional EDU digunakan, pilih yang lebih tinggi dari INT×5 atau EDU×5 untuk peluang dasar. Memahami dialek yang tidak jelas atau variasi kuno suatu bahasa adalah Sulit. Dokumen atau percakapan yang kompleks atau panjang dengan beberapa poin penting mungkin memerlukan beberapa lemparan bahasa untuk dipahami, satu untuk setiap poin. Lihat tabel **Kelancaran Bahasa** untuk detail tentang bagaimana peringkat keterampilan memengaruhi komunikasi dan kelancaran.

##### Kelancaran Bahasa

| Peringkat Keterampilan | Kelancaran |
| :---- | :---- |
| 01–05 | Hanya tahu beberapa lusin kata, bisa menghitung sampai 10. Hanya dapat mengomunikasikan ide sederhana. Komunikasi kompleks (Menawar, Membujuk, dll.) tidak mungkin dilakukan. Sebagai contoh, “Makanan. Tolong jual saya.” |
| 6–25 | Menyampaikan permintaan sederhana, cukup untuk dipahami dan bertahan hidup sehari-hari. Komunikasi kompleks (Menawar, Membujuk, dll.) adalah Sulit. Sebagai contoh, “Berapa uang untuk paha domba ini?” |
| 26–50 | Komunikasi terjamin. Pembicara dapat berbicara lebih baik daripada anak kecil, menyampaikan sebagian besar ide. Keterampilan komunikasi tidak dibatasi, tetapi tindakan tertentu mungkin masih Sulit. Sebagai contoh, “Berapa? Tapi domba ini jauh lebih murah kemarin!” |
| 51–75 | Memungkinkan pembicara menceritakan kisah, saga, lagu, dll. Idiom dan lelucon menjadi dapat diakses. Bisa lolos sebagai penduduk asli. Komunikasi kompleks sekarang dilempar pada peringkat keterampilan penuh pembicara. Sebagai contoh, “Lihat potongan ini! Domba itu sudah busuk sebelum disembelih dan jelas bahkan tidak sebanding dengan keringat buruh yang membawanya ke sini.” |
| 76–00 | Bahasa penyair, filsuf, ilmuwan, dan diplomat. Pembicara tidak hanya bisa lolos sebagai penduduk asli, tetapi mereka juga bisa meniru satu atau lebih dialek lokal. Jargon dan bentuk ucapan yang tidak jelas datang dengan mudah. Sebagai contoh, “Tentu penilaian spesimen provender ini dapat dievaluasi kembali mengingat kondisinya yang membusuk.” |

### Mendengar

**Peluang Dasar:** 25%

**Kategori:** Persepsi

Gunakan keterampilan ini untuk mendengar, menafsirkan, dan memahami suara, baik gumaman di balik pintu tertutup, bisikan diam-diam, langkah kaki diam-diam, dan ranting yang patah. Gunakan keterampilan ini untuk melihat apakah suara membangunkan karakter yang sedang tidur, atau untuk melihat apakah mereka dapat mengenali suara atau suara tertentu. Jangkauan normal untuk mendengar suara Manusia berbicara dengan volume sopan adalah 15 meter—suara yang lebih keras membawa lebih jauh, dan suara yang lebih tenang memiliki jangkauan yang lebih pendek. Kegagalan Mutlak berarti kesalahpahaman atau ketidaktahuan total tentang suatu suara, sementara keberhasilan menghasilkan kesadaran dan informasi tentang sumbernya.

**Catatan Sistem:** Mendengar adalah keterampilan aktif dan pasif: karakter Anda dapat mencoba Mendengar suara mencurigakan, atau gamemaster dapat meminta lemparan Mendengar bila diperlukan. Lawan Mendengar dengan Siluman untuk menyelesaikan upaya menyelinap melewati pendengar yang waspada atau tidak waspada. Gamemaster dapat memilih untuk melakukan lemparan Mendengar secara rahasia dan memberi tahu Anda hasilnya, untuk menghindari memberikan apa pun jika lemparan gagal.

### Literasi (berbagai)

**Peluang Dasar:** 00% atau setara dengan Bahasa (Sendiri/Lain) (lihat di bawah)

**Kategori:** Mental

Kemampuan membaca dan menulis dianggap remeh di era modern: di zaman sebelumnya, membaca dan menulis adalah produk pendidikan elit dan tanda perbedaan atau status. Dalam masyarakat modern, peluang dasar Literasi sama dengan Bahasa (Sendiri) atau (Lain) karakter Anda kecuali jika diinginkan lain. Kegagalan Mutlak berarti kesalahpahaman yang membawa bencana, gagal berarti makna hilang, keberhasilan memberikan informasi atau nuansa kritis, bahkan ketenaran untuk karya yang dihasilkan.

**Spesialisasi:** Berdasarkan bahasa. Setiap spesialisasi berlaku untuk bahasa tertulis tunggal; bahasa tambahan adalah spesialisasi Literasi terpisah. Dalam masyarakat modern, Literasi selalu setara dengan keterampilan Bahasa Anda.

**Catatan Sistem:** Memahami dialek yang tidak jelas atau variasi kuno bahasa tertulis adalah Sulit. Dokumen kompleks atau panjang (seperti buku) dengan beberapa poin penting mungkin memerlukan beberapa lemparan Literasi, satu untuk setiap poin.

### Seni Bela Diri

**Peluang Dasar:** 01%

**Kategori:** Tempur

Ini mewakili studi dan penguasaan gaya bertarung tertentu. Gunakan keterampilan ini untuk melakukan gerakan dan serangan spesifik yang menyebabkan kerusakan maksimal pada target. Seni Bela Diri selalu digunakan dalam kombinasi dengan keterampilan lain: Berkelahi, Senjata Jarak Dekat, Senjata Proyektil, atau Bergulat, sebagaimana mestinya. Keterampilan ini tidak diperlukan untuk melakukan serangan tersebut—ini mewakili tingkat pelatihan yang lebih besar yang memungkinkan efektivitas yang lebih besar.

**Spesialisasi:** Berdasarkan tradisi seni bela diri: tinju, capoeira, escrima, anggar, jeet kun do, karate, kenjutsu, kickboxing, kung fu, kyujutsu, pugilisme, savate, dll. Gamemaster dapat membatasi jenis keterampilan tempur yang dapat dipasangkan dengan Seni Bela Diri, seperti hanya mengizinkan Senjata Proyektil (Busur) dengan kyujutsu, misalnya.

**Efek:** Seni Bela Diri tidak dilempar secara terpisah saat melakukan serangan. Lempar terhadap keterampilan utama dan bandingkan hasilnya dengan keterampilan Seni Bela Diri. Hasilnya memiliki efek ini:

* **Kegagalan Mutlak:** Gunakan hasil Kegagalan Mutlak yang sesuai (lihat tabel serangan dan Kegagalan Mutlak di *Bab Enam: Tempur*).
* **Kegagalan:** Jika hasil yang dilempar di atas peringkat keterampilan Seni Bela Diri tetapi masih sama dengan atau di bawah keterampilan tempur, serangan mengenai secara normal. Serangan yang dilempar di atas keterampilan tempur meleset, terlepas dari peringkat keterampilan Seni Bela Diri.
* **Keberhasilan:** Jika hasil yang dilempar kurang dari atau sama dengan Seni Bela Diri dan keterampilan tempur, lempar untuk kerusakan dasar dua kali dan jumlahkan hasilnya. Pengubah kerusakan karakter Anda tidak pernah digandakan.
* **Khusus:** Seperti di atas. Untuk keberhasilan khusus, lemparan kerusakan dasar tambahan ditambahkan ke efek keberhasilan khusus keterampilan tempur, jika ada.
* **Kritis:** Seperti di atas. Jika lemparan tempur adalah keberhasilan kritis, perlindungan Zirah target (alami atau dikenakan) diabaikan, dan lemparan kerusakan dasar tambahan ditambahkan ke efek keberhasilan khusus keterampilan tempur, jika ada.

**Catatan Sistem:** Keterampilan Seni Bela Diri bekerja bersama dengan satu keterampilan tempur lainnya, dan biasanya dengan kisaran spesialisasi yang sempit untuk keterampilan tersebut. Kekuatan ini dapat digunakan bersama dengan mutasi Persenjataan Alami dan kekuatan Pertarungan Tanpa Senjata.

Jika beberapa serangan dilakukan dalam satu putaran, penyerang harus mengalokasikan keterampilan Seni Bela Diri yang sesuai, membaginya sesuai keinginan di antara beberapa serangan, atau menerapkannya hanya pada satu serangan.

Sebagai contoh, *jika karakter Anda memiliki Berkelahi 120% dan Seni Bela Diri 46%, dan melakukan dua serangan pada 60% dalam satu putaran, Anda harus memilih untuk membagi keterampilan Seni Bela Diri antara kedua serangan ini (masing-masing 23%, atau kombinasi apa pun yang sama dengan 46%), atau memilih untuk menerapkannya hanya pada salah satu serangan.*

Karena kekuatannya yang relatif, gamemaster dapat memutuskan bahwa peringkat keterampilan awal dalam keterampilan ini dibatasi, bahwa peningkatan pada keterampilan ini dapat dibatasi pada pelatihan daripada pengalaman normal, bahwa pelatihan mungkin sangat sulit diperoleh, atau ketiga kondisi ini.

### Pengobatan

**Peluang Dasar:** 05%

**Kategori:** Mental

Gunakan keterampilan ini untuk mendiagnosis dan mengobati cedera serius, mengobati penyakit, melawan keracunan, dan membuat rekomendasi kesehatan umum, serta melakukan operasi dan menasihati perawatan jangka panjang untuk kondisi kesehatan kronis.

**Efek:** Pengobatan memiliki hasil spesifik:

* **Kegagalan Mutlak:** Pasien menerima 1 poin hit umum (untuk cedera) saat mengobati cedera, atau kehilangan poin karakteristik (untuk racun atau penyakit), dan kondisi mereka tetap tidak berubah.
* **Kegagalan:** Kondisi pasien tidak berubah, dan upaya kedua hanya dapat dilakukan pada hari berikutnya.
* **Keberhasilan:** Pilih salah satu dari efek berikut (pilihan pemain):
  * Efek apa pun dari lemparan Pertolongan Pertama yang berhasil.
  * Tingkat penyembuhan pasien berlipat ganda dari 1D3 menjadi 2D3 poin hit per minggu.
  * Kerusakan karakteristik atau poin hit berkelanjutan dari racun atau penyakit dihentikan.
  * Karakter yang diracuni atau sakit distabilkan dan pulih 1D3–1 poin hit atau poin karakteristik per minggu. Hasil ini hanya dapat dipilih sekali per minggu perawatan.
* **Khusus:** Seperti di atas, tetapi cedera disembuhkan sebesar 2D3 poin hit, atau 1D3 poin karakteristik pulih.
* **Kritis:** Seperti di atas, tetapi luka atau cedera disembuhkan sebesar 1D3+3 poin hit, atau 1D3+1 poin karakteristik pulih.

**Catatan Sistem:** Setiap penggunaan Pengobatan berlaku untuk satu penyakit: setelah dirawat dan benar-benar sembuh, lemparan Pengobatan yang berhasil lebih lanjut tidak berpengaruh. Namun, luka atau penyakit lain dapat diobati. Kondisi tidak aman atau tidak sehat dapat membuat lemparan Pengobatan menjadi Sulit. Peralatan atau obat khusus dapat menambahkan hingga +20% pada peringkat keterampilan, sementara teknologi medis futuristik (semprotan kulit buatan, pemindai medis, penyembuh nanotech) membuat lemparan Pengobatan menjadi Mudah. Mengobati penyakit umum dan tidak mengancam jiwa adalah Otomatis.

### Senjata Jarak Dekat (berbagai)

**Peluang Dasar:** Sesuai spesialisasi senjata

**Kategori:** Tempur

Gunakan keterampilan ini untuk menggunakan jenis senjata tertentu dalam pertarungan tangan kosong, baik menyerang maupun menangkis.

**Spesialisasi:** Spesialisasi setara dengan kelas senjata. Lihat **Kelas Senjata** dari *Bab Delapan: Peralatan*. Setiap spesialisasi hanya berlaku untuk senjata dari kategori senjata tersebut: tetapi gamemaster dapat mengizinkan penggunaan kelas senjata serupa seolah-olah Sulit. Jika berhasil, lakukan pemeriksaan pengalaman. Jika lemparan pengalaman berhasil, mulai keterampilan baru dengan spesialisasi pada peringkat tersebut, ditambah pengalaman yang diperoleh.

**Efek:** Lihat **Matriks Serangan dan Pertahanan** dan **Tabel Kegagalan Mutlak Serangan Senjata Jarak Dekat** serta **Tabel Kegagalan Mutlak Tangkisan Senjata Jarak Dekat** di *Bab Enam: Tempur* untuk efek spesifik.

**Catatan Sistem:** Karakter Anda dapat mencoba lebih dari satu tangkisan per putaran, tetapi setiap lemparan setelah yang pertama dibuat dengan kumulatif –30%. Atas kebijaksanaan gamemaster, karakter Anda dapat menggunakan senjata dari kategori senjata yang terkait atau serupa, dengan lemparan yang Sulit.

### Senjata Proyektil (berbagai)

**Peluang Dasar:** Sesuai spesialisasi senjata

**Kategori:** Tempur

Gunakan ini untuk menggunakan senjata proyektil yang digerakkan sendiri yang digenggam tangan seperti busur, busur silang, kapak lempar, tombak, bintang lempar, lembing, blowgun, pistol tombak, atau bahkan bumerang. Untuk granat, gunakan Melempar.

**Spesialisasi:** Spesialisasi setara dengan kelas senjata. Lihat **Kelas Senjata** di *Bab Delapan: Peralatan*. Setiap spesialisasi hanya berlaku untuk senjata dari kategori senjata tersebut: tetapi gamemaster dapat mengizinkan penggunaan kelas senjata serupa seolah-olah Sulit. Jika berhasil, lakukan pemeriksaan pengalaman. Jika lemparan pengalaman berhasil, mulai keterampilan baru dengan spesialisasi pada peringkat tersebut, ditambah pengalaman yang diperoleh.

**Efek:** Lihat **Matriks Serangan dan Pertahanan** dan **Tabel Kegagalan Mutlak Senjata Proyektil** untuk efek spesifik. Panah dan senjata runcing biasanya menusuk. Jika Anda ingin tahu ke mana serangan senjata proyektil yang meleset, lihat keterampilan Melempar.

**Catatan Sistem:** Semua senjata proyektil memiliki persyaratan STR dan DEX minimum untuk menggunakan senjata tersebut, sebagaimana tercantum dalam *Bab Delapan: Peralatan*. Biasanya, pengubah kerusakan penyerang dibagi dua untuk senjata proyektil, dan untuk senjata yang sepenuhnya digerakkan sendiri (blowgun, busur silang, dll.), pengubah kerusakan tidak diterapkan. Senjata Proyektil dapat digunakan untuk serangan terarah.

### Navigasi

**Peluang Dasar:** 10%

**Kategori:** Persepsi

Gunakan keterampilan ini untuk menemukan jalan ke tujuan baik dengan kompas, peta laut, bintang, atau hanya menggunakan persepsi, ingatan, dan intuisi sebagai panduan. Kegagalan Mutlak berarti seseorang tersesat tanpa harapan dan lemparan berikutnya oleh siapa pun untuk tidak tersesat menjadi *Sulit*. Gagal berarti penundaan dan laju perjalanan yang melambat, sementara keberhasilan berarti menemukan jalan seseorang secara efektif dan bahkan membuat kemajuan yang lebih baik dari yang diharapkan.

**Catatan Sistem:** Lakukan lemparan keterampilan Navigasi setiap empat jam perjalanan yang sedang berlangsung. Penggunaan peta atau kompas dapat meningkatkan peringkat keterampilan hingga +20%. Menavigasi di lanskap tanpa fitur (di laut, dalam kegelapan atau cuaca buruk, di gurun, atau di hutan lebat) tanpa instrumen atau peta adalah Sulit. Alat bantu navigasi canggih (teknologi layanan posisi global, dll.) atau keakraban dengan area tersebut membuat upaya Navigasi menjadi Mudah.

### Tangkisan (berbagai)

**Peluang Dasar:** Sesuai spesialisasi senjata

**Kategori:** Tempur

Gunakan keterampilan ini untuk menangkis serangan yang masuk jika tidak ditangani dengan keterampilan lain. Keterampilan ini dibahas secara rinci dalam Tangkisan dan Matriks Serangan dan Pertahanan. Gamemaster dapat memutuskan bahwa serangan tertentu tidak dapat ditangkis, seperti dari penyerang yang jauh lebih besar (dua kali atau lebih SIZ bek, misalnya) atau ketika penyerang menggunakan serangan area atau sapuan.

**Spesialisasi:** Spesialisasi setara dengan kelas senjata atau perisai. Lihat **Kelas Senjata** di *Bab Delapan: Peralatan*. Setiap spesialisasi hanya berlaku untuk senjata atau perisai dari kategori senjata tersebut: tetapi gamemaster dapat mengizinkan penggunaan kelas senjata serupa seolah-olah Sulit. Jika berhasil, lakukan pemeriksaan pengalaman. Jika lemparan pengalaman berhasil, mulai keterampilan baru dengan spesialisasi pada peringkat tersebut, ditambah pengalaman yang diperoleh.

**Efek:** Lihat **Matriks Serangan dan Pertahanan** dan **Tabel Kegagalan Mutlak Tangkisan Senjata Jarak Dekat** di *Bab Enam: Tempur* untuk efek spesifik.

**Catatan Sistem:** Lebih dari satu tangkisan dapat dicoba per putaran, tetapi setiap lemparan setelah yang pertama dikenakan penalti –30% pada peringkat keterampilan perisai atau tangkisan. Keterampilan ini dapat diabaikan demi menggunakan senjata utama atau keterampilan perisai, jika diinginkan. Atas kebijaksanaan gamemaster, seseorang dapat menangkis dengan senjata atau perisai dari kategori terkait atau serupa, dengan lemparan yang Sulit.

### Pertunjukan (berbagai)

**Peluang Dasar:** 05%

**Kategori:** Komunikasi

Gunakan keterampilan ini untuk menampilkan musik, baik berakting, menari, memainkan alat musik, menyanyi, atau beberapa bentuk ekspresi artistik lainnya menggunakan pertunjukan. Kegagalan Mutlak adalah penghinaan, gagal tidak menginspirasi, sementara keberhasilan menghasilkan perhatian positif, sorak-sorai, dan mungkin pengakuan kritis dan imbalan finansial. Jika direkam dan didistribusikan, itu mungkin terus menginspirasi dan menghasilkan jauh setelah pertunjukan asli.

**Spesialisasi:** Akting, Memimpin Orkestra, Menari, Menyulap, Berpidato, Memainkan Alat Musik (sub-spesialisasi), Melafalkan, Ritual, Menyanyi, dll.

**Catatan Sistem:** Penggunaan Pertunjukan (Ritual) yang berhasil dapat menambah kekuatan sihir atau memberikan manfaat tambahan, atas kebijaksanaan gamemaster.

### Membujuk

**Peluang Dasar:** 15%

**Kategori:** Komunikasi

Gunakan keterampilan ini untuk meyakinkan seseorang bahwa suatu ide atau keyakinan tertentu adalah benar, tepat, atau dapat diterima, terlepas dari pendapat asli mereka (jika ada) baik melalui logika Socratic, debat terstruktur, orasi berapi-api, empati, atau intimidasi kasar. Pendengar dapat diyakinkan untuk mengambil tindakan atau mengabulkan permintaan. Seperti Bicara Cepat, kebenaran tidak relevan. Tidak seperti Bicara Cepat, efek dari persuasi yang berhasil berlangsung tanpa batas waktu, sampai lemparan Membujuk lainnya atau pengalaman dramatis menggeser pendapat subjek. Kegagalan Mutlak berarti subjek mungkin menjadi bermusuhan, merangkul kebalikan dari pendapat yang diinginkan. Gagal tidak meyakinkan. Keberhasilan mengubah pendapat mereka, dan keberhasilan yang unggul menghasilkan hasil yang lebih baik.

**Catatan Sistem:** Menggunakan Membujuk melintasi hambatan bahasa (jika kedua pembicara memiliki di bawah 50% dalam bahasa tersebut) adalah Sulit, seperti halnya meyakinkan seseorang tentang sesuatu yang bertentangan dengan kepribadian atau keyakinan inti mereka. Gamemaster dapat menghargai argumen yang efektif atau roleplaying yang baik dengan bonus hingga +20%. Tergantung pada keadaan, Membujuk dapat memakan waktu menit, jam, atau bahkan hari, dan upaya yang diperpanjang mungkin memerlukan beberapa lemparan untuk diselesaikan.

### Pilot (berbagai)

**Peluang Dasar:** 01%

**Kategori:** Fisik

Gunakan keterampilan ini untuk mengarahkan, bermanuver, dan mengendalikan kendaraan melalui udara, di atas atau di bawah air, atau di luar angkasa. Kendaraan yang lebih besar mungkin memerlukan beberapa pilot, dan lemparan harus dilakukan setiap empat jam saat tidak dalam pertempuran, dan sekali per putaran selama pertempuran atau saat melakukan manuver kendaraan.

**Spesialisasi:** Pesawat Terbang, Kapal Udara, Balon, Perahu, Helikopter, Hovercraft, Pesawat Jet, Paket Jet, Pesawat Luar Angkasa, Kapal Selam, dll. Atas kebijaksanaan gamemaster, pilot dapat mengemudikan kapal dengan spesialisasi serupa seolah-olah Sulit. Jika berhasil, lakukan pemeriksaan pengalaman. Jika lemparan pengalaman berhasil, mulai keterampilan baru dengan spesialisasi pada peringkat tersebut, ditambah pengalaman yang diperoleh.

**Efek:** Sebagian besar penerbangan sehari-hari adalah Otomatis—lemparan hanya dalam kondisi buruk atau untuk situasi luar biasa. Kegagalan Mutlak hampir pasti menyebabkan mesin berhenti atau bahkan jatuh, gagal berarti tujuan tidak tercapai dalam jangka waktu yang diinginkan, keberhasilan sampai di tempat yang diinginkan, dan keberhasilan yang unggul menghasilkan penerbangan dan hasil yang lebih ahli. Lihat **Pertempuran Udara** dan **Pengejaran** untuk informasi lebih lanjut, dan **Tabel Masalah Pengejaran** untuk kecelakaan.

### Proyeksi

**Peluang Dasar:** DEX×2%

**Kategori:** Fisik

Keterampilan opsional ini digunakan oleh karakter pengguna kekuatan untuk menargetkan serangan berbasis kekuatan. Ini dijelaskan dalam *Bab Empat: Kekuatan* dan hanya digunakan dalam permainan dengan kekuatan yang sesuai.

### Psikoterapi

**Peluang Dasar:** 01%

**Kategori:** Mental

Gunakan keterampilan ini untuk mengevaluasi dan mengobati penyakit mental atau trauma, terutama memulihkan poin Kewarasan yang hilang. Dalam praktiknya, Psikoterapi dapat digunakan untuk meresepkan obat dan medikasi, yang dari waktu ke waktu dapat menangguhkan, mengobati, dan bahkan membalikkan psikosis atau penyakit mental yang melemahkan. Medikasi dan pertanyaan intensif dapat menyelidiki kedalaman jiwa yang rusak dan membawa kembali harapan dari teror dan keputusasaan. Lihat **Kewarasan** untuk informasi lebih lanjut.

**Efek:** Psikoterapi memiliki efek spesifik:

* **Kegagalan Mutlak:** Salah satu dari berikut ini terjadi (pilihan gamemaster):
  * Pasien harus melakukan lemparan Kewarasan baru, untuk kehilangan 1/1D3 SAN.
  * Tingkatkan peringkat POW dari salah satu kegilaan tak terbatas pasien sebesar +1.
  * Pasien menukar satu kegilaan tak terbatas dengan yang lain: fobia mungkin berkurang, tetapi dengan biaya kecanduan obat penstabil suasana hati.
* **Kegagalan:** Perawatan tidak berpengaruh. Pasien tetap sama.
* **Keberhasilan:** Perawatan mulai berlaku, dengan salah satu efek berikut:
  * Setelah sebulan perawatan, pasien mendapatkan kembali 1D3–1 poin SAN. Keterampilan ini tidak dapat meningkatkan SAN pasien di atas POW×5 dikurangi penalti yang sesuai untuk pengetahuan tentang horor yang menghujat atau tidak terkatakan. Lihat Kewarasan Maksimum untuk detailnya.
  * Sumber kegilaan tak terbatas didiagnosis dengan benar.
  * Setelah sumber kegilaan tak terbatas didiagnosis, POW-nya dapat dikurangi sebesar –1 setelah satu bulan perawatan. Lemparan terpisah harus dilakukan untuk setiap penyakit mental yang diobati.
* **Khusus**: Seperti di atas, tetapi perolehan SAN adalah 1D3 atau pengurangan POW adalah –2.
* **Kritis**: Seperti di atas, tetapi perolehan SAN adalah 1D3+1 atau pengurangan POW adalah –1D3+1.

**Catatan Sistem:** Keterampilan ini terutama untuk permainan di mana Kewarasan digunakan. Perawatan memerlukan empat hingga lima jam terapi per bulan per kegilaan tak terbatas. Atas kebijaksanaan gamemaster, Psikoterapi dapat digunakan untuk memanipulasi sifat kepribadian seperti halnya skor SAN, menggesernya secara positif atau negatif dengan jumlah poin yang sama.

### Perbaikan (berbagai)

**Peluang Dasar:** 15%

**Kategori:** Manipulasi

Peralatan dan objek lain rusak atau berhenti bekerja. Gunakan Perbaikan untuk memperbaikinya atau membuat sedikit modifikasi dan adaptasi.

**Spesialisasi:** Listrik, Elektronik, Teknik, Pembangkit Listrik Tenaga Air, Mekanik, Struktural, Kuantum, dll.

**Efek:** Kegagalan Mutlak merusak barang lebih lanjut dan bahkan dapat menyakiti orang yang melakukan upaya perbaikan (1D3 poin hit untuk keduanya, misalnya) dan membuat perbaikan lebih lanjut menjadi Sulit. Gagal tidak menyelesaikan apa pun, keberhasilan memulihkan 1D3 poin hit dan barang tersebut menjadi dapat digunakan kembali atau kembali berfungsi. Keberhasilan yang unggul menghasilkan manfaat serupa, mungkin menggunakan keterampilan Pertolongan Pertama dan penilaian gamemaster sebagai panduan. Kritis bahkan dapat meningkatkan barang dengan cara kecil, seperti menambah total poin hit, poin Zirah, atau menambah bonus kecil saat digunakan.

### Riset

**Peluang Dasar:** 25%

**Kategori:** Persepsi

Gunakan keterampilan ini untuk menemukan dan mengidentifikasi sumber informasi potensial di perpustakaan, arsip, atau jaringan. Lemparan yang berhasil menghasilkan informasi yang diinginkan dari buku, dokumen, artikel, bank data, arsip, atau situs web, dengan keberhasilan yang unggul memberikan informasi tambahan atau lebih spesifik. Meskipun berbagai jenis sumber daya, prinsipnya sama. Upaya keterampilan ini memakan waktu empat jam secara normal, meskipun metode elektronik jauh lebih cepat. Kegagalan Mutlak menyebabkan kesalahpahaman yang membawa bencana dan menghasilkan informasi yang salah. Gagal membuang waktu tanpa hasil yang solid atau memberikan jumlah informasi minimum setelah dua kali jumlah waktu normal.

**Catatan Sistem:** Gamemaster disarankan untuk membuat lemparan Riset yang gagal memakan waktu lebih lama, memberikan informasi yang sedikit menyesatkan atau rumit, membunyikan semacam alarm atau perhatian yang tidak diinginkan, atau kemunduran lain daripada sekadar tidak membuahkan hasil. Kegagalan jarang menyenangkan dalam permainan, dan untuk misteri, petunjuk yang tidak dapat diakses adalah jalan buntu yang dramatis.

### Menunggang (berbagai)

**Peluang Dasar:** 05%

**Kategori:** Komunikasi

Gunakan keterampilan ini untuk menunggangi, menangani, dan merawat jenis hewan hidup tertentu. Dengan Menunggang karakter Anda dapat mengarahkan tunggangan melalui manuver khusus atau medan kasar, membujuknya ke kecepatan tertinggi, atau untuk tetap terkendali jika ia berdiri atau tersandung. Kegagalan Mutlak menyebabkan kehilangan kendali atas tunggangan, mungkin mengakibatkan terlempar ke tanah untuk 1D6 poin hit kerusakan (umum atau ke lokasi hit acak). Lemparan Melompat yang berhasil mengurangi 1D6 dari total kerusakan, sementara tunggangan lari dengan kecepatan tertinggi. Gagal menyebabkan hewan itu mogok, dan keberhasilan menghasilkan ia melakukan seperti yang diinginkan atau bahkan melebihi harapan. Keberhasilan yang unggul mungkin menghasilkan bonus untuk tindakan tempur saat menunggang.

**Spesialisasi:** Berdasarkan jenis hewan. Lihat *Bab Sebelas: Makhluk* untuk contoh tunggangan. Atas kebijaksanaan gamemaster, jenis hewan serupa dapat ditunggangi seolah-olah Sulit. Jika berhasil, lakukan pemeriksaan pengalaman. Jika lemparan pengalaman berhasil, mulai keterampilan baru dengan spesialisasi pada peringkat tersebut, ditambah pengalaman yang diperoleh.

**Catatan Sistem:** Selesaikan balapan antara jenis hewan yang sama menggunakan lemparan Menunggang yang berlawanan. Keterampilan Tempur dan Menunggang harus 50%+ untuk bertarung secara normal dalam pertarungan berkuda. Jika keterampilan Menunggang kurang dari 50%, lemparan Menunggang diperlukan setiap putaran untuk menjaga kendali tunggangan. Jika keterampilan tempur kurang dari 50%, semua serangan adalah Sulit. Bahkan jika kedua keterampilan lebih tinggi dari 50%, gunakan yang lebih rendah dari kedua peringkat keterampilan untuk serangan dan tangkisan. Jika tunggangan jatuh, roboh, atau mati, perlakukan penunggang seolah-olah mereka telah terlempar. Lihat **Pertarungan Berkuda** untuk informasi lebih lanjut.

### Sains (berbagai)

**Peluang Dasar:** 01%

**Kategori:** Mental

Gunakan keterampilan ini untuk mengetahui informasi dan prinsip ilmiah dari bidang tertentu, dan untuk mengamati fenomena serta menarik kesimpulan berbasis data. Informasi terkait mencakup sejarah sains, keakraban dengan peralatan dan metodologi, serta kesadaran akan tokoh-tokoh penting dan aliran pemikiran dalam spesialisasi tersebut.

**Spesialisasi:** Astronomi, Ilmu Perilaku, Biologi, Botani, Kimia (atau Alkimia), Kriptografi, Genetika, Geologi, Matematika, Meteorologi, Sejarah Alam, Farmakologi, Fisika, Planetologi, Psikologi, Mekanika Kuantum, Xenobiologi, Zoologi, dll. Atas kebijaksanaan gamemaster, keterampilan dengan spesialisasi terkait dapat dilempar seolah-olah Sulit, meskipun lemparan yang berhasil tidak menghasilkan pemeriksaan pengalaman atau kesempatan untuk memulai keterampilan baru dengan spesialisasi tersebut.

**Catatan Sistem:** Banyak spesialisasi Sains memerlukan peralatan khusus, instrumen pengukur, komputer, atau bahkan laboratorium. Lemparan sains dapat memakan waktu mulai dari 30 menit hingga berminggu-minggu atau berbulan-bulan, atas kebijaksanaan gamemaster: empat jam untuk eksperimen adalah rata-rata yang baik. Mencoba lemparan Sains tanpa waktu atau peralatan yang cukup adalah Sulit atau Mustahil.

### Perasaan

**Peluang Dasar:** 10%

**Kategori:** Persepsi

Keterampilan ini terutama mengatur rasa, sentuhan, dan bau; tetapi dapat mencakup beberapa indra lain yang kurang dikenal (orientasi fisik, gravitasi, tekanan dan keseimbangan, suhu, dll.). Gunakan untuk memperhatikan atau mengidentifikasi jenis stimulus tertentu yang bukan penglihatan (Melihat) atau suara (Mendengar), seperti menemukan barang dalam kegelapan dengan sentuhan, mengidentifikasi bau atau rasa yang misterius, merasakan patah tulang rambut, dll. Atas kebijaksanaan gamemaster, ini mungkin bahkan mencakup "indra keenam" atau intuisi. Kegagalan Mutlak secara menyesatkan atau mengungkapkan ketidaktahuan total tentang stimulus, gagal adalah kegagalan sederhana untuk memperhatikan, dan berbagai tingkat keberhasilan memberikan persepsi yang lebih akurat.

**Catatan Sistem:** Seperti Mendengar dan Melihat, Perasaan adalah keterampilan aktif dan pasif: Anda dapat meminta karakter Anda untuk melakukan lemparan Perasaan, atau gamemaster dapat meminta lemparan dalam situasi yang sesuai. Stimulus yang kuat atau akrab adalah lemparan Mudah untuk Perasaan, sementara yang samar atau tidak akrab adalah Sulit.

### Perisai

**Peluang Dasar:** Sesuai jenis perisai

**Kategori:** Tempur

Gunakan keterampilan ini untuk memblokir serangan yang masuk dengan perisai atau alat lain yang sesuai. Semua blok perisai mengikuti aturan yang sama untuk menangkis: lihat **Perisai** dan **Tangkisan** di *Bab Enam: Tempur*. Seseorang dapat menyerang dengan perisai, memperlakukan keterampilan ini seolah-olah itu adalah keterampilan Senjata Jarak Dekat. Peringkat kerusakan diberikan untuk perisai dalam daftar senjata. Perisai melakukan hasil khusus penghancuran, dapat menyebabkan knockback, dan dapat digunakan untuk serangan terarah.

**Efek:** Lihat **Matriks Serangan dan Pertahanan** dan **Tabel Kegagalan Mutlak Tangkisan Senjata Jarak Dekat**.

**Catatan Sistem:** Blok perisai adalah tangkisan. Karakter dapat melakukan lebih dari satu lemparan perisai dalam satu putaran, tetapi setiap lemparan tambahan dikenakan penalti –30% pada peringkat keterampilan perisai atau tangkisan. Perisai besar dapat memberikan perlindungan terhadap serangan proyektil atau bahkan senjata api.

### Kecekatan Tangan

**Peluang Dasar:** 05%

**Kategori:** Manipulasi

Gunakan keterampilan ini untuk memanipulasi benda kecil (dadu, kartu, koin, dompet, surat, dll.) dengan cara yang menyesatkan atau tanpa menarik perhatian, seperti saat mencopet, curang kartu, atau melakukan sihir panggung. Kegagalan Mutlak sangat jelas: apakah barang tersebut dijatuhkan, atau calon pencuri tertangkap basah dengan tangan di saku target. Gagal berarti tidak ada yang dicapai, dan target yang tidak curiga yang berhasil melakukan lemparan Ide mungkin menyadari bahwa mereka sedang ditipu.

**Catatan Sistem:** Keterampilan ini dilawan oleh Melihat jika perlu. Setelah objek dipegang atau diambil, gunakan Bersembunyi untuk menyembunyikannya dari pencarian terpadu.

### Melihat

**Peluang Dasar:** 25%

**Kategori:** Persepsi

Gunakan keterampilan ini untuk mencari detail yang berpotensi penting di dalam ruangan, menemukan kompartemen tersembunyi, memperhatikan detail yang sekilas, mengenali musuh yang menyamar, atau melihat musuh yang bersembunyi. Kegagalan Mutlak adalah persepsi salah atau kesan keliru yang sangat mengganggu, dan berbagai tingkat keberhasilan memberikan informasi yang lebih berguna dan relevan.

**Catatan Sistem:** Melihat adalah keterampilan aktif dan pasif: karakter Anda dapat melakukan lemparan Melihat untuk mencari atau memeriksa secara aktif, atau gamemaster dapat meminta lemparan Melihat untuk melihat apakah karakter Anda memperhatikan detail yang sekilas. Bersembunyi atau penyembunyian yang disengaja memerlukan tes Melihat vs. Bersembunyi yang berlawanan untuk diselesaikan. Melihat juga sering dilawan vs. Penyamaran atau Kecekatan Tangan. Dalam kegelapan total, Melihat adalah Mustahil, dan cahaya redup atau kabut tebal membuatnya Sulit.

### Status

**Peluang Dasar:** 15%

**Kategori:** Komunikasi

Status adalah ukuran reputasi, kredensial profesional, kelas sosial, kemakmuran, ketenaran, dan bahkan kehormatan pribadi. Gunakan keterampilan ini untuk memengaruhi orang lain dengan melempar beban, seperti meyakinkan bank untuk memberikan pinjaman tunai atau memaafkan utang, mengintimidasi karakter non-pemain untuk memberikan bantuan, meyakinkan vendor untuk puas dengan I.O.U., atau menekan pihak berwenang untuk menjaga masalah tetap diam. Kegagalan Mutlak adalah faux pas dengan besaran terbesar, menyebabkan rasa malu, menarik perhatian yang tidak diinginkan, dan memiliki kebalikan mutlak dari efek yang diinginkan. Keberhasilan menghasilkan hasil yang dicari atau bahkan bantuan dan pertimbangan tambahan. Status bukan kekayaan, bagaimanapun, dan sementara mereka sering dihubungkan, mereka tidak secara intrinsik terkait satu sama lain.

**Spesialisasi:** Setiap spesialisasi mewakili kelas sosial, profesi, kelompok sosial, atau area pengaruh tunggal.

**Catatan Sistem:** Status hanya beroperasi dalam area tertentu, berdasarkan spesialisasi, dan menggunakannya di luar area pengaruh adalah Sulit. Status tidak ditingkatkan dengan pengalaman, pelatihan, atau peningkatan lainnya. Gamemaster harus menawarkan Status sebagai imbalan atas layanan setia, tindakan berani, atau prestasi luar biasa di antara skenario. Satu poin per petualangan masuk akal, dengan tindakan luar biasa atau pencapaian meningkatkan perolehan menjadi 1D3, 1D4, atau mungkin bahkan 1D6 poin. Demikian pula, kegagalan, skandal, pelanggaran ringan, dan ketidakpantasan sosial dapat menurunkan Status. Status dapat digunakan untuk mengesankan, tetapi juga berfungsi sebagai sarana untuk menentukan ketenaran. Karakter Anda melempar melawan Status orang lain untuk mengidentifikasi mereka, dan karakter non-pemain dapat melempar melawan Status karakter Anda untuk menentukan apa, jika ada, yang mereka ketahui tentang karakter tersebut.

> ### Keterampilan Status, Status Sosial, & Kekayaan Karakter
>
> Seperti yang dijelaskan dalam **Membuat Karakter** dan **Profesi Karakter**, Status dapat secara dramatis memengaruhi kedudukan sosial dan kekayaan. Di bawah ini adalah tiga tabel yang menetapkan peringkat sosial dan kekayaan rata-rata berdasarkan Status, dengan kekayaan maksimum untuk Status tersebut. Saat seseorang mendapatkan Status, posisi dan kekayaan mereka kemungkinan akan meningkat. Gamemaster Anda harus merevisi tabel ini atau membuat yang baru, sesuai keinginan.
>
> ##### Status Prasejarah
>
> | Status | Peringkat Sosial | Peringkat Kekayaan | Kekayaan Maksimum |
> | :---- | :---- | :---- | :---- |
> | 01–15 | Budak (Kelas Bawah) | Miskin | Miskin |
> | 16–95 | Anggota Suku (Kelas Menengah) | Miskin | Rata-rata |
> | 96–00 | Bangsawan (Kelas Atas) | Berada | Kaya |
>
> ##### Status Kuno/Zaman Kegelapan/Abad Pertengahan/Kekaisaran
>
> | Status | Peringkat Sosial | Peringkat Kekayaan | Kekayaan Maksimum |
> | :---- | :---- | :---- | :---- |
> | 01–39 | Kelas Bawah | Destitut | Destitut |
> | 40–43 | Kelas Menengah Bawah | Miskin | Rata-rata |
> | 44–47 | Kelas Menengah | Rata-rata | Rata-rata |
> | 48–49 | Kelas Menengah Atas | Rata-rata | Rata-rata |
> | 50–75 | Kelas Atas | Berada | Berada |
> | 76–95 | Bangsawan | Berada | Kaya |
> | 96–00 | Monarki | Kaya | Kaya |
>
> ##### Status Victoria/Barat/Pulp/Modern
>
> | Status | Peringkat Sosial | Peringkat Kekayaan | Batas Kekayaan |
> | :---- | :---- | :---- | :---- |
> | 01–14 | Kelas Bawah | Destitut | Miskin |
> | 15–29 | Kelas Bawah | Miskin | Rata-rata |
> | 30–39 | Kelas Menengah Bawah | Rata-rata | Berada |
> | 40–75 | Kelas Menengah | Rata-rata | Berada |
> | 76–95 | Kelas Menengah Atas | Berada | Kaya |
> | 96–00 | Kelas Atas | Kaya | Kaya |
>
> Status juga dapat memengaruhi peralatan awal yang dimulai oleh karakter Anda. Atas kebijaksanaan gamemaster, Anda dapat melakukan lemparan Status untuk menentukan apakah karakter Anda memulai dengan perlengkapan yang lebih baik daripada tingkat kekayaan dan profesi mereka. Lihat Peralatan Awal di *Bab Delapan: Peralatan*. Lemparan status yang dilakukan sebelum permainan tidak memenuhi syarat untuk pemeriksaan pengalaman.

### Siluman

**Peluang Dasar:** 10%

**Kategori:** Fisik

Gunakan keterampilan ini untuk menyelinap melalui area tanpa menarik perhatian. Tidak diperlukan penutup, tetapi langkah ringan, kepercayaan diri, dan disiplin semuanya diperlukan. Untuk menghindari terlihat saat tidak bergerak, gunakan Bersembunyi. Kegagalan Mutlak menarik perhatian yang tidak diinginkan, gagal berarti tidak ada kemajuan yang dibuat, dan karakter jelas bagi siapa pun yang melihat, sementara keberhasilan menawarkan tingkat penyembunyian yang berturut-turut.

**Catatan Sistem:** Siluman mengasumsikan pergerakan lambat. Bergerak lebih jauh dari 1/2 MOV seseorang dalam satu putaran tempur adalah Sulit. Faktor lingkungan (suara sekitar, karpet tebal, lantai berderit, dll.) atau perlengkapan (sepatu bersol lunak, Zirah berisik, peralatan besar, dll.) dapat memodifikasi lemparan Siluman, atas kebijaksanaan gamemaster. Siluman dilawan oleh Melihat atau Mendengar, sebagaimana mestinya.

### Strategi

**Peluang Dasar:** 01%

**Kategori:** Mental

Gunakan keterampilan ini untuk memandu pasukan dalam pertempuran, dari regu kecil hingga pasukan atau armada besar, mengatur dan mengoordinasikan logistik untuk pasukan, merencanakan dan melaksanakan perusahaan militer skala besar, dan menentukan apa yang mungkin dipikirkan oleh ahli strategi lain. Keterampilan ini juga mengasumsikan pengetahuan dasar tentang sejarah militer dan taktik, serta tokoh-tokoh penting di dalamnya. Kegagalan Mutlak mewakili bencana militer dan mungkin menyebabkan kekalahan total, sementara berbagai tingkat keberhasilan memberikan keuntungan strategis dan mungkin berarti bonus bagi moral dan efektivitas tempur.

**Catatan Sistem:** Pertunangan militer besar-besaran harus diselesaikan dalam bentuk abstrak, dengan lemparan Strategi yang berlawanan antara pemimpin yang berlawanan yang memengaruhi hasil (beberapa lemparan mungkin diperlukan untuk pertunangan yang lebih besar atau lebih lama). Menggunakan Strategi dalam kondisi yang tidak menguntungkan (kalah jumlah, persediaan buruk, dll.) adalah Sulit. Keterampilan ini sering digunakan bersama Perintah: Strategi mewakili rencana, Perintah adalah seberapa jelas itu disampaikan kepada mereka yang harus mengeksekusinya.

### Berenang

**Peluang Dasar:** 25%

**Kategori:** Fisik

Gunakan keterampilan ini untuk bergerak melalui atau di bawah air. Umumnya, lemparan Berenang hanya boleh digunakan untuk situasi berbahaya atau menekan. Mengapung, mendayung anjing, dan menginjak air dalam kondisi normal adalah *Otomatis*. Kegagalan Mutlak berarti bahwa Anda mulai tenggelam (lihat *Tersedak, Tenggelam, dan Asfiksia*). Setelah karakter Anda mulai tenggelam, hanya lemparan Berenang yang *Sulit* atau bantuan luar yang dapat mengembalikan mereka ke permukaan untuk bernapas lagi. Keberhasilan berarti manuver normal di air, sementara keberhasilan yang unggul berarti kecepatan dan kemampuan manuver tambahan.

**Catatan Sistem:** Di perairan tenang, lemparan Berenang hanya diperlukan sekali per giliran. Berenang di perairan kasar memerlukan pemeriksaan Berenang setiap putaran. Berenang sambil membawa sesuatu memerlukan lemparan Usaha atau lemparan resistensi STR, tergantung pada situasinya. Peralatan khusus (pelampung, sirip renang, dll.) menawarkan pengubah hingga +20%. Berenang secara aktif menghabiskan 1 poin kelelahan per giliran. Selesaikan balapan di air sebagai lemparan Berenang yang berlawanan, menghabiskan 1 poin kelelahan per putaran. Kurangi ENC apa pun yang dimiliki perenang dari peringkat Berenang mereka. Jika ENC mereka melebihi STR mereka, mereka tenggelam dan harus mengurangi beban atau tenggelam. Karakter yang mengenakan Zirah memiliki penalti yang diterapkan pada Berenang (lihat **Zirah**). Untuk melepas Zirah di air, lakukan lemparan Berenang sekali per putaran tempur per poin nilai tertinggi Zirah (maksimum lemparan jika nilai Zirah acak digunakan). Zirah alami atau Zirah yang diberikan oleh kekuatan tidak memiliki penalti di air.

Dalam pertarungan jarak dekat di bawah air, penyerang menggunakan yang lebih rendah dari peringkat keterampilan tempur atau Berenang mereka untuk menyelesaikan serangan, dan mencoba Menghindar adalah Sulit. Hanya senjata penusuk yang berguna—semua yang lain melakukan 1D3–1 poin kerusakan per hit. Hambatan air juga menambahkan 3 poin nilai Zirah ke target apa pun. Lihat **Pertarungan Bawah Air** untuk informasi lebih lanjut.

### Mengajar

**Peluang Dasar:** 10%

**Kategori:** Komunikasi

Gunakan keterampilan ini setiap kali karakter Anda mencoba melatih atau mengajarkan informasi atau teknik kepada orang lain, menggunakan kuliah, latihan, atau sparring fisik. Kegagalan Mutlak bersifat mundur dan menyebabkan siswa mungkin kehilangan 1D2 poin keterampilan jika mereka tidak melakukan lemparan Ide untuk mengenali pengajaran yang buruk, sementara keberhasilan mengajarkan sesuai aturan normal untuk pelatihan dan keberhasilan yang unggul meningkatkan pelatihan keterampilan menjadi 1D6 (khusus) dan 1D6+2 (kritis).

**Catatan Sistem:** Untuk mengajarkan keterampilan tertentu, guru harus memiliki dua kali peringkat keterampilan siswa, atau peringkat keterampilan 50% (mana yang lebih rendah). Lihat **Pelatihan Keterampilan** untuk detail lebih lanjut.

### Keterampilan Teknis (berbagai)

**Peluang Dasar:** 05%

**Kategori:** Mental

Gunakan keterampilan ini untuk mengoperasikan dan memelihara perangkat teknologi canggih seperti sistem komputer, robot, atau perangkat tinggi lainnya. Ini juga dapat digunakan untuk memodifikasi perangkat guna meningkatkan kinerjanya atau menggunakannya untuk tujuan baru di luar parameternya. Kegagalan Mutlak berpotensi merusak perangkat atau membuat macet dengan cara tertentu, sementara keberhasilan dan keberhasilan yang unggul menyebabkan mekanisme berfungsi sebagaimana dirancang, atau melampaui harapan.

**Spesialisasi:** Jam, Komputer, Siber, Elektronik, Robotika, Sensor, Mesin Pengepungan, Jebakan, dll.

**Catatan Latar:** Keterampilan Teknis dalam latar fantasi atau sejarah mencakup jebakan, perangkat jam atau roda gigi, mesin pengepungan, dll.

### Melempar

**Peluang Dasar:** 25%

**Kategori:** Fisik

Gunakan keterampilan ini untuk mengenai target dengan benda yang dilempar, atau untuk melempar senjata yang tidak seimbang untuk dilempar (seperti melempar pedang besar atau perisai). Ini mencakup benda yang dilempar secara improvisasi, granat, batang dinamit, senjata yang biasanya tidak dilempar, atau bukan senjata. Laso juga dicakup oleh keterampilan ini, serta menangkap barang. Jika senjata dibuat untuk dilempar, gunakan Senjata Proyektil.

**Efek:** Melempar memiliki efek spesifik:

* **Kegagalan Mutlak:** Objek dijatuhkan atau hilang. Dalam pertempuran, gunakan **Tabel Kegagalan Mutlak Senjata Proyektil**.
* **Kegagalan:** Objek meleset dan pergi 1D6 meter ke arah acak.
* **Keberhasilan:** Objek mendarat di tempat yang dituju. Lempar kerusakan secara normal, jika ada.
* **Khusus:** Objek mendarat di tempat yang dituju dan memberikan jenis kerusakan khusus yang sesuai dengan bentuknya (lihat **Keberhasilan Khusus**).
* **Kritis:** Objek mendarat di tempat yang dituju dengan hasil yang luar biasa, mengabaikan Zirah dan memberikan kerusakan kritis, jika sesuai.

**Catatan Sistem:** Tambahkan 1/2 pengubah kerusakan (bulatkan ke atas) ke kerusakan dasar senjata lempar. Sebagian besar barang dapat dilempar sejauh STR pelempar dalam meter, dan jarak di luar itu adalah Sulit. Objek yang dilempar dapat diarahkan.

### Melacak

**Peluang Dasar:** 10%

**Kategori:** Persepsi

Gunakan keterampilan ini untuk mengidentifikasi jejak atau mengikuti jejak—baik orang, kendaraan, atau hewan—di atas tanah, melalui semak, salju, dan melintasi medan yang rusak. Kegagalan Mutlak kehilangan jejak dan mungkin pelacak itu sendiri. Berbagai tingkat keberhasilan menghasilkan hasil yang lebih baik, termasuk berpotensi dapat menilai informasi tentang subjek yang dilacak atau memprediksi secara akurat ke mana mereka pergi..

**Catatan Sistem:** Lemparan Melacak memakan waktu 1D3 menit, dan upaya pelacakan yang diperpanjang mungkin memerlukan beberapa lemparan. Tergantung pada lingkungan, lemparan mungkin Sulit (malam hujan) atau Mudah (hari bersalju cerah). Jika jejak hilang, menemukannya kembali adalah Sulit. Mencoba menyembunyikan jejak seseorang adalah lemparan Melacak yang berlawanan. Peralatan canggih dapat menambahkan hingga +20% pada peringkat keterampilan.